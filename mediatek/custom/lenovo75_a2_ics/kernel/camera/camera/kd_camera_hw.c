...
u32 pinSet[3][8] = {
    ...
    if (On) {
        ...
        if( (0 == pinSetIdx) || (2 == pinSetIdx) )
        {
            // main and 2nd main camera

            // DOVDD = 1.8V
            if(TRUE != hwPowerOn(CAMERA_POWER_VCAM_D, VOL_1800,mode_name))     
            {
                PK_DBG("[CAMERA SENSOR] Fail to set 1.8V for CAMERA_POWER_VCAM_D\n");
                //return -EIO;
                goto _kdCISModulePowerOn_exit_;
            }
            mdelay(1); 
            ...
            if(TRUE != hwPowerOn(CAMERA_POWER_VCAM_A, VOL_2800,mode_name))
            {
                PK_DBG("[CAMERA SENSOR] Fail to set 2.8V for CAMERA_POWER_VCAM_A\n");
                //return -EIO;
                goto _kdCISModulePowerOn_exit_;
            }                    
            mdelay(1); 
            ...
            #if 0 
            // DVDD = 0V for using internal DVDD
            if(TRUE != hwPowerDown(CAMERA_POWER_VCAM_D2,mode_name))
            {
                // mtk00719:  igore error due to fail to power down un-power case 
                /*
                PK_DBG("[CAMERA SENSOR] Fail to set 0V for CAMERA_POWER_VCAM_D2\n");
                //return -EIO;
                goto _kdCISModulePowerOn_exit_;
                */
            }
            #else
            // DVDD = 1.5V for using external DVDD            
            if(TRUE != hwPowerOn(CAMERA_POWER_VCAM_D2, VOL_1500,mode_name))
            {
                PK_DBG("[CAMERA SENSOR] Fail to set 1.5V for CAMERA_POWER_VCAM_D2\n");
                //return -EIO;
                goto _kdCISModulePowerOn_exit_;
            }        
            #endif
        ...
        else
        {
            ...
            if(TRUE != hwPowerOn(CAMERA_POWER_VCAM_D, VOL_1800,mode_name))     
            {
                PK_DBG("[CAMERA SENSOR] Fail to set 1.8V to CAMERA_POWER_VCAM_D\n");
                //return -EIO;
                goto _kdCISModulePowerOn_exit_;
            }
            // DVDD = 1.8V
            ...
            {
                PK_DBG("[CAMERA SENSOR] Fail to set 1.8V to CAMERA_POWER_VCAM_A2\n");
                //return -EIO;
                goto _kdCISModulePowerOn_exit_;
            }
            // AVDD = 2.8V
            if(TRUE != hwPowerOn(CAMERA_POWER_VCAM_A, VOL_2800,mode_name))     
            {
                PK_DBG("[CAMERA SENSOR] Fail to set 2.8V to CAMERA_POWER_VCAM_A\n");
                //return -EIO;
                goto _kdCISModulePowerOn_exit_;
            }        
        }
        ...
        if(TRUE != hwPowerDown(CAMERA_POWER_VCAM_A,mode_name)) {
            PK_DBG("[CAMERA SENSOR] Fail to OFF CAMERA_POWER_VCAM_A\n");
            //return -EIO;
            goto _kdCISModulePowerOn_exit_;
        }
        ...
        {
            if(TRUE != hwPowerDown(CAMERA_POWER_VCAM_A2,mode_name)) {
                PK_DBG("[CAMERA SENSOR] Fail to OFF CAMERA_POWER_VCAM_A2\n");
                //return -EIO;
                goto _kdCISModulePowerOn_exit_;
            }     	
       }
       ...
       if(TRUE != hwPowerDown(CAMERA_POWER_VCAM_D, mode_name)) {
            PK_DBG("[CAMERA SENSOR] Fail to OFF CAMERA_POWER_VCAM_D\n");
            //return -EIO;
            goto _kdCISModulePowerOn_exit_;
       }
       ...
       {        
            if(TRUE != hwPowerDown(CAMERA_POWER_VCAM_D2,mode_name))
            {
                // mtk00719: igore error due to fail to power down un-power case         
                /*
                PK_DBG("[CAMERA SENSOR] Fail to OFF CAMERA_POWER_VCAM_D2r\n");
                //return -EIO;
                goto _kdCISModulePowerOn_exit_;
                */
            }                    
