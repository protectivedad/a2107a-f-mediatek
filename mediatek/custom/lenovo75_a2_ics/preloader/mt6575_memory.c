...
void dram_column_size_test (void) 
{
	unsigned int i,u4reg,u4cnt;
	unsigned int *u4Addr=0;
	unsigned int u4Pattern=0x12345678;
	 if ((emi_settings[0].type & 0xF00) != 0x000)
		{
			print ("dram column size detect does not support in MCP\n");
			return;
		}
	print ("Dram column size test\n");
	u4reg=(*(volatile unsigned int *)0xC0003000)&0xFFFFFF00;
	 *(volatile unsigned int *)0xC0003000=(u4reg|0x2E); // 8Gb size (for different column type memory using)
  ...
  u4Addr=0x800;
	for(i=0;i<8;i++) 
	{
    *(u4Addr+i)=(u4Pattern+i);    
	}
	u4Addr=0x0;
	u4cnt=0;
	for(i=0;i<8;i++) 
	{
    if((*(u4Addr+i))==(u4Pattern+i))
    	{
    		u4cnt++;	
    	}
	 }
	 if(u4cnt == 8)
	 {	 	  
	 	  *(volatile unsigned int *)0xC0003000=(u4reg|0x1A);// 4Gb
	 	  print ("Dram size 4Gb\n");
	 }	
	 else if (u4cnt == 0)
	 {
	 	  *(volatile unsigned int *)0xC0003000=(u4reg|0x2E); // 8Gb
	 	  print ("Dram size 8Gb\n");
	 }	
	 else
	 {
	 ...
	 emi_settings[0].EMI_CONA_VAL= (*(volatile unsigned int *)0xC0003000);
}
...
// --------------------------------------------------------
void
mt6575_mem_init (void)
{
#if !(CFG_FPGA_PLATFORM)
    mt6575_set_emi ();
#endif

dram_column_size_test();

#if MEM_TEST
    {
        int i = 0;
        if ((i = complex_mem_test (0x0, MEM_TEST_SIZE)) == 0)
        {
