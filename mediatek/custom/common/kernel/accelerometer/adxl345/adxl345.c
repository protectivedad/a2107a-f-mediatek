...
static struct i2c_board_info __initdata i2c_adxl345={ I2C_BOARD_INFO("ADXL345", 0x53)};
...
/*----------------------------------------------------------------------------*/
static int adxl345_gpio_config(void)
{
...
static int adxl345_init_client(struct i2c_client *client, int reset_cali)
{
	...
	if(res != ADXL345_SUCCESS) 
	{
	    GSE_ERR("ADXL345_SetIntEnable error\n");
		return res;
	}
...
static int ADXL345_ReadSensorData(struct i2c_client *client, char *buf, int bufsize)
{
	...
	int res = 0;	
	struct adxl345_i2c_data *obj = obj_i2c_data; //(struct adxl345_i2c_data*)i2c_get_clientdata(client);
	client = obj->client;
	//u8 databuf[20];
	...
	switch (command)
	{
		...
		case SENSOR_GET_DATA:
			...
			else
			{
				...
				err = ADXL345_ReadSensorData(priv->client, buff, ADXL345_BUFSIZE);
				if(!err)
				{
...
//       unsigned long arg)
static long adxl345_unlocked_ioctl(struct file *file, unsigned int cmd,
       unsigned long arg)
{
...
static int adxl345_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if((err = adxl345_create_attr(&adxl345_gsensor_driver.driver)))
	{
...
static int adxl345_i2c_remove(struct i2c_client *client)
{
	...
	if((err = adxl345_delete_attr(&adxl345_gsensor_driver.driver)))
	{
...
static int adxl345_probe(struct platform_device *pdev) 
{
	...
	ADXL345_power(hw, 1);
	//adxl345_force[0] = hw->i2c_num;//modified
...
{
	GSE_FUN();
	i2c_register_board_info(0, &i2c_adxl345, 1);
