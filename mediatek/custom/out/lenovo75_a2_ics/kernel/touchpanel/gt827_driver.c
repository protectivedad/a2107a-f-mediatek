...
{             
    ...
    *(volatile u16 *) TPD_RESET_PIN_ADDR = temp;
    ...
    printk("MediaTek touch panel sets VGP2 hwPowerOn!!!\n");
...
int gt827_unlocked_ioctl ( struct inode *inode, struct file *filp, unsigned int cmd, unsigned long arg)
{
...
static int touch_event_handler(void *unused) {
    ...
    int x, y, id, size, temp, max_finger_id = 0, finger_num = 0;
    static u8 id_mask = 0;	
	u8 keyStatus, touchBufferStatus, lastKey = 0x00;
    u16 cur_mask;
