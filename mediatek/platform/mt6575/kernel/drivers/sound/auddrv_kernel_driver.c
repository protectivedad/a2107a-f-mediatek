...
#ifndef MT6575_AFE_MCU_IRQ_LINE
#define MT6575_AFE_MCU_IRQ_LINE    MT6575_AFE_MCU_IRQ_ID
#endif
...
static unsigned long long gLastTaskletTime = 0;
...
void AudDrv_Test_Print(void)
{
  ...
  // xlog_printk(ANDROID_LOG_INFO, "Sound","AMP_Flag         = %d\n",AMP_Flag);
   xlog_printk(ANDROID_LOG_INFO, "Sound","Afe_Mem_Pwr_on   = %d\n",Afe_Mem_Pwr_on);
...
void AudDrv_Clk_On(void)
{
   ...
   if(Aud_AFE_Clk_cntr == 0 )
   {
      ...
      kal_uint32 afe_clk = (aud_top & 0x4);
      if(afe_clk == 0x4){
         xlog_printk(ANDROID_LOG_INFO, "Sound","+AudDrv_Clk_On AFE clk(%d)(%x)\n",Aud_AFE_Clk_cntr,afe_clk);
      }
   }
...
{
   kal_uint32 aud_top = 0;
   kal_uint32 afe_clk = 0;
   ...
   afe_clk = (aud_top & 0x4);
   if(afe_clk == 0x4){
      xlog_printk(ANDROID_LOG_INFO, "Sound","+AudDrv_DL1_Stream_Standby AFE clk(%x) Chia \n",aud_top);
   }
   else{
...
void AudDrv_Init_DL1_Stream(kal_uint32 Afe_Buf_Length)
{
   ...
   afe_clk = (aud_top & 0x4);
   if(afe_clk == 0x4){
      xlog_printk(ANDROID_LOG_INFO, "Sound","+AudDrv_Init_DL1_Stream AFE clk(%x) Chia \n",aud_top);
   }
   else{
   ...
   align = (ptr & 0x1f);
   if(align != 0){
      xlog_printk(ANDROID_LOG_INFO, "Sound","AudDrv_Init_DL1_Stream dam dam not 32bytes align ptr:%x \n",ptr);
   }
   // 32 bytes align
/*
   ptr   = AFE_dl_Control->rBlock.pucPhysBufAddr;
...
void AudDrv_Reset_DL1_Stream_Buf(void)
{
    if(AFE_dl_Control->rBlock.u4BufferSize > 0 && AFE_dl_Control->rBlock.pucVirtBufAddr != NULL){
        ...
        kal_uint32 afe_clk = (aud_top & 0x4);
        if(afe_clk == 0x4){
           xlog_printk(ANDROID_LOG_INFO, "Sound","+AudDrv_Reset_DL1_Stream_Buf AFE clk(%x) Chia \n",aud_top);
        }
        else{
...
void AudDrv_Do_Tasklet(void)  // interrupt tasklet
{
   ...
   //kal_int32 Temp_Afe_consumed_bytes = 0;
   ...
   kal_int32 u4Afe_irq1_cnt    = 0;
   unsigned long long t_TaskletTime = 0;
   ...
   if(u4Afe_irq1_cnt > 0){
       t_TaskletTime = sched_clock(); // in ns (10^9)
   }
   ...
   if(AudDrv_Check(Afe_Block)){
      xlog_printk(ANDROID_LOG_INFO, "Sound","AudDrv_Do_Tasklet: u4BufferSize= %d, Afe_Block->u4DMAReadIdx = %d   Afe_Block->u4DataRemained  = %d Error!!\n"
...
HW_READ_DATA_AGAIN:
   ...
   else if( (u4Afe_irq_status&0xf) & IRQ_MCU_DAI_SET_CLR)  // BT DAI
   {
      kal_uint32 aud_top = 0;
      kal_uint32 afe_clk = 0;
      xlog_printk(ANDROID_LOG_INFO, "Sound","AudDrv_Do_Tasklet IRQ_MCU_DAI_SET_CLR \n");
      ...
      afe_clk = (aud_top & 0x4);
      if(afe_clk == 0x4){
        xlog_printk(ANDROID_LOG_INFO, "Sound","+AudDrv_Do_Tasklet AFE clk(%x) Chia \n",aud_top);
      }
      else{
   ...
   {
      xlog_printk(ANDROID_LOG_INFO, "Sound","AudDrv_Do_Tasklet IRQ_MCU_DAI_RST_CLR \n");
   ...
   else
   {
        ...
        if(HW_Cur_ReadIdx == 0){
          xlog_printk(ANDROID_LOG_ERROR, "Sound","Aud DL Tasklet error, AFE_DL1_CUR, NULL pointer ");
          goto EXIT_TASKLET;
        }
...
*/
        ...
        //   xlog_printk(ANDROID_LOG_INFO, "Sound","AudDrv_Do_Tasklet consumed_bytes:%d \n",Afe_consumed_bytes);
        if((Afe_consumed_bytes & 0x1f) != 0 ){
          //xlog_printk(ANDROID_LOG_INFO, "Sound","[Auddrv] DMA address is not aligned 32 bytes \n");
        }
   }
   if(u4Afe_irq1_cnt > 0){
       if(gLastTaskletTime != 0)
       {
           unsigned long long tTimeInterval = (t_TaskletTime - gLastTaskletTime);
           if (tTimeInterval > ((unsigned long long)m_Interrupt_time)*2*1000000){
              xlog_printk(ANDROID_LOG_VERBOSE,"Sound","Tasklet is delayed (%ld)ns\n",tTimeInterval);
           }
       }
       gLastTaskletTime = t_TaskletTime;
   }
   if((Afe_Block->u4DataRemained <= Afe_consumed_bytes)||(u4Afe_irq1_cnt > 1))
   {
      ...
      kal_uint32 afe_clk = (aud_top & 0x4);
      if(afe_clk == 0x4){
         xlog_printk(ANDROID_LOG_INFO, "Sound","+AudDrv_Do_Tasklet1 AFE clk(%x) Chia \n",aud_top);
      }
      else{
         memset(Afe_Block->pucVirtBufAddr,0,Afe_Block->u4BufferSize);
      }
      xlog_printk(ANDROID_LOG_VERBOSE, "Sound","+Tasklet underflow ReadIdx:%x WriteIdx:%x, DataRemained:%x, HW_cons:%x \n",
              Afe_Block->u4DMAReadIdx,Afe_Block->u4WriteIdx,Afe_Block->u4DataRemained,Afe_consumed_bytes);
      if (u4Afe_irq1_cnt > 1)
          xlog_printk(ANDROID_LOG_ERROR,"Sound","+Tasklet is blocked %d times\n",u4Afe_irq1_cnt-1);
      ...
      xlog_printk(ANDROID_LOG_VERBOSE, "Sound","-Tasklet 2underflow ReadIdx:%x, WriteIdx:%x, DataRemained:%x \n",
   ...
   else
   {
      ...
      PRINTK_AUDDRV("Tasklet normal ReadIdx:%x ,DataRemained:%x, WriteIdx:%x \n",
...
void  AudDrv_I2Sin_Tasklet(void)
{
   ...
   if(HW_Cur_WriteIdx == 0){
      xlog_printk(ANDROID_LOG_ERROR, "Sound","AudDrv_I2Sin_Tasklet, Error, AFE_I2S_CUR, NULL pointer ");
      return;
   }

   if(I2Sin_Block->u4BufferSize == 0){
      xlog_printk(ANDROID_LOG_ERROR, "Sound","AudDrv_I2Sin_Tasklet: u4BufferSize=0, Error");
      return;
   }
   ...
   {
      xlog_printk(ANDROID_LOG_INFO, "Sound","AudDrv_I2Sin_Tasklet buffer overflow u4DMAReadIdx:%x, u4WriteIdx:%x, u4DataRemained:%x, u4BufferSize:%x \n",
...
{
   kal_uint32 u4RegValue = Afe_Get_Reg(AFE_IR_STATUS);
...
static long AudDrv_ioctl(struct file *fp, unsigned int cmd, unsigned long arg)
{
   ...
   if((cmd!=AUDDRV_ENABLE_ATV_I2S_GPIO)&&(cmd!=AUDDRV_DISABLE_ATV_I2S_GPIO)) //prevent additional time cost
   {
       AudDrv_Clk_On();
   }
   switch(cmd)
...
// for DL1 output SRC
      ...
      case START_DL1_STREAM:
      {
        ...
        if(!Flag_Aud_DL1_SlaveOn)
        {
         ...
         gLastTaskletTime = 0;//reset tasklet timer
...
// record the Speech/Background/Recording state in kernel
      case SET_AUDIO_STATE:
      {
         ...
         xlog_printk(ANDROID_LOG_INFO, "Sound","AudDrv bBgsFlag:%d,bRecordFlag:%d,bSpeechFlag:%d,bTtyFlag:%d,bVT:%d,bAudio:%d \n",
         SPH_Ctrl_State.bBgsFlag,
         SPH_Ctrl_State.bRecordFlag,
         SPH_Ctrl_State.bSpeechFlag,
         SPH_Ctrl_State.bTtyFlag,
...
// for debug
      ...
      {
          xlog_printk(ANDROID_LOG_INFO, "Sound","AudDrv AUDDRV_BEE_IOCTL arg:%ld \n",arg);
          //LouderSPKSound(300);
          break;
      }
      ...
      default:
      {
      	xlog_printk(ANDROID_LOG_INFO, "Sound","AudDrv Fail IOCTL command (%d)\r\n", cmd);
      	break;
      }
	}
...
//   HW_Cur_ReadIdx = Afe_Get_Reg(AFE_DL1_CUR);  // for debugging
    if(m_first_write == 0){
        ...
        xlog_printk(ANDROID_LOG_INFO, "Sound","aud_top  =%u Aud_AFE_Clk_cntr = %d Afe_Mem_Pwr_on = %d Aud_ANA_Clk_cntr = %d",
            Afe_Get_Reg(AUDIO_TOP_CON0),Aud_AFE_Clk_cntr,Afe_Mem_Pwr_on,Aud_ANA_Clk_cntr);
        m_first_write ++;
    }
   ...
   while(count)
   {
      ...
      if(copy_size != 0)
      {
         if(Afe_Block->u4WriteIdx + copy_size < Afe_Block->u4BufferSize ) // copy once
         {
            if(!access_ok(VERIFY_READ,data_w_ptr,copy_size)){
                xlog_printk(ANDROID_LOG_INFO, "Sound","AudDrv_write 0ptr invalid data_w_ptr=%x, size=%d",(kal_uint32)data_w_ptr,copy_size);
                xlog_printk(ANDROID_LOG_INFO, "Sound","AudDrv_write u4BufferSize=%d, u4DataRemained=%d",Afe_Block->u4BufferSize, Afe_Block->u4DataRemained);
            }
            else
            {
               // need check AFE clk
               ...
               kal_uint32 afe_clk = (aud_top & 0x4);
               if(afe_clk == 0x4){
                  xlog_printk(ANDROID_LOG_INFO, "Sound","+AudDrv_write AFE clk(%x) Chia \n",aud_top);
               }
               else{
         ...
         else  // copy twice
         {
            ...
            else
            {
               ...
               if(afe_clk == 0x4){
                  ...
                  {
                     xlog_printk(ANDROID_LOG_INFO, "Sound","AudDrv_write Fail 1 copy from user");
                     return -1;
                  }
               }
            }
            ...
            if(!access_ok (VERIFY_READ,data_w_ptr+size_1, size_2)){
                xlog_printk(ANDROID_LOG_INFO, "Sound","AudDrv_write 2ptr invalid data_w_ptr=%x, size_1=%d, size_2=%d",(kal_uint32)data_w_ptr,size_1,size_2);
                xlog_printk(ANDROID_LOG_INFO, "Sound","AudDrv_write u4BufferSize=%d, u4DataRemained=%d",Afe_Block->u4BufferSize, Afe_Block->u4DataRemained);
            }
            else
            {
               ...
               if(afe_clk == 0x4){
                  ...
                  {
                     xlog_printk(ANDROID_LOG_INFO, "Sound","AudDrv_write Fail 2 copy from user");
                     return -1;
                  }
               }
            }
      ...
      if(count != 0)
      {
         ...
         if( (t2) > (( ((unsigned long long)m_Interrupt_time)*11*1000000/8) )) // 1.x times interrupt timer. unit: ns
         {
         ...
         else if( (t2) > (  (((unsigned long long)m_Interrupt_time)+4)*1000000 )) // check the int timer+4ms
         {
         ...
         if((ret <= 0) || (ret1 < 0))
         {
             ...
             if(irq1_mcu_on == 0x1)
             {
                 ...
                 if( (SPH_Ctrl_State.bSpeechFlag==false) && (SPH_Ctrl_State.bBgsFlag==false) && (SPH_Ctrl_State.bTtyFlag==false)
                     ...
                     AFE_dl_Control->rBlock.u4fsyncflag    = false;
                     spin_unlock_bh(&auddrv_lock);
                     xlog_printk(ANDROID_LOG_INFO, "Sound","AudDrv_write timeout,restart \n");
                     msleep(1);
                     Afe_Set_Reg(AFE_DAC_CON0,0x2,0x2);
                     Afe_Set_Reg(AFE_IRQ_CON,0x1,0x1);
                 }else{
                     xlog_printk(ANDROID_LOG_INFO, "Sound","AudDrv_write timeout, not restart, bBgsFlag:%d,bRecordFlag:%d,bSpeechFlag:%d,bTtyFlag:%d,bVT:%d,bAudio:%d \n",
                            SPH_Ctrl_State.bBgsFlag,
                            SPH_Ctrl_State.bRecordFlag,
                            SPH_Ctrl_State.bSpeechFlag,
                            SPH_Ctrl_State.bTtyFlag,
...
{
   xlog_printk(ANDROID_LOG_INFO, "Sound","AudDrv_Suspend_Clk_On Aud_AFE_Clk_cntr:%d \n",Aud_AFE_Clk_cntr);
   spin_lock_bh(&auddrv_lock);
   if(Aud_AFE_Clk_cntr>0)
   {
#ifdef USE_PM_API
...
{
   xlog_printk(ANDROID_LOG_INFO, "Sound","AudDrv_Suspend_Clk_Off Aud_AFE_Clk_cntr:%d \n",Aud_AFE_Clk_cntr);
   spin_lock_bh(&auddrv_lock);
   if(Aud_AFE_Clk_cntr>0)
   {
      // Disable AFE clock
...
static int AudDrv_probe(struct platform_device *dev)
{
   ...
   // register IRQ line
   ret = request_irq(MT6575_AFE_MCU_IRQ_LINE, AudDrv_IRQ_handler, IRQF_TRIGGER_LOW/*IRQF_TRIGGER_FALLING*/, "Afe_ISR_Handle", dev);
   if(ret < 0 ){
...
static int AudDrv_suspend(struct platform_device *dev, pm_message_t state)  // only one suspend mode
{
   ...
   xlog_printk(ANDROID_LOG_INFO, "Sound","AudDrv_suspend \n");
 ...
 //Temp to disable[Charlie]
      ...
      xlog_printk(ANDROID_LOG_INFO, "Sound","AudDrv_suspend Afe_Mem_Pwr_on:%d, Aud_AFE_Clk_cntr:%d, Aud_ADC_Clk_cntr:%d, Aud_I2S_Clk_cntr:%d \n",Afe_Mem_Pwr_on,Aud_AFE_Clk_cntr, Aud_ADC_Clk_cntr, Aud_I2S_Clk_cntr);
...
static int AudDrv_resume(struct platform_device *dev) // wake up
{
   ...
   if(IsSuspen == true)
   {
      xlog_printk(ANDROID_LOG_INFO, "Sound","AudDrv_resume Afe_Mem_Pwr_on:%d, Aud_AFE_Clk_cntr:%d\n",Afe_Mem_Pwr_on,Aud_AFE_Clk_cntr);
