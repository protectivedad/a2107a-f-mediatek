...
#include <rtc-mt6575.h>		/* custom file */

#define XLOG_MYTAG	"Power/RTC"
#define RTC_NAME	"mt6575-rtc"
...
static irqreturn_t rtc_irq_handler(int irq, void *dev_id)
{
	...
	disable_irq_nosync(MT6575_RTC_IRQ_ID);
...
static int rtc_pdrv_probe(struct platform_device *pdev)
{
	...
	r = request_irq(MT6575_RTC_IRQ_ID, rtc_irq_handler, IRQF_TRIGGER_LOW,
	                RTC_NAME, NULL);
	if (r) {
...
module_param(rtc_show_alarm, bool, 0644);

MODULE_AUTHOR("Terry Chang <terry.chang@mediatek.com>");
MODULE_DESCRIPTION("MT6575 RTC Driver v3.0");
MODULE_LICENSE("GPL");
