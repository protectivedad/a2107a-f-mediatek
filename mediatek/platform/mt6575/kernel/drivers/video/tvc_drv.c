...
#define TV_DBG(fmt, arg...)      xlog_printk( ANDROID_LOG_DEBUG, "TV/TVC", "[DBG]: %s() "fmt"\n", __FUNCTION__, ##arg)
#define TV_INFO(fmt, arg...)     xlog_printk( ANDROID_LOG_INFO, "TV/TVC", "[INFO]: %s() "fmt"\n", __FUNCTION__, ##arg)
#define TV_WARN(fmt, arg...)     xlog_printk( ANDROID_LOG_WARN, "TV/TVC", "[WARN]: %s() "fmt"\n", __FUNCTION__, ##arg)
#define TV_ERROR(fmt, arg...)    xlog_printk( ANDROID_LOG_ERROR, "TV/TVC", "[ERR]: %s(): %s@%d "fmt"\n", __FUNCTION__, __FILE__,__LINE__,##arg)
#define TV_FATAL(fmt, arg...)    xlog_printk( ANDROID_LOG_FATAL, "TV/TVC", "[FATAL]: %s(): %s@%d "fmt"\n", __FUNCTION__, __FILE__,__LINE__,##arg)
...
static irqreturn_t _TVC_InterruptHandler(int irq, void *dev_id)
{
    TVC_REG_IRQ_STS status = TVC_REG->IRQ_STATUS;

    //mt6575_irq_mask(MT6575_TVC_IRQ_ID);
    //disable_irq(MT6575_TVC_IRQ_ID);
        ...
        while (TVC_REG->REG_UPDATE.REG_RDY)
        {
			...
			if (_tvcContext.isTvcEnabled) {
            	long ret = wait_event_interruptible_timeout(_tvcContext.tvc_checkline,
                                                        ...
                                                        WAIT_TIMEOUT);
            	if (0 == ret) {
                	TV_WARN("Wait for TVC checkline IRQ timeout!!!");
            	}
			} else {
			    /*
...
static TVC_STATUS _AllocateInternalSRAM(void)
{
    ...
    if (0 == _tvcContext.pfhDmaBufPA) {
        TV_ERROR("allocate TVC internal SRAM failed");
...
TVC_STATUS TVC_Init(void)
{
    ...
    {
        TV_ERROR("fail to request TVC irq");
        return TVC_STATUS_ERROR;
    }
...
TVC_STATUS TVC_Disable(void)
{
    ...
    {
        TV_WARN("TVC is disabled, but disable again");
        return TVC_STATUS_OK;
    }
...
TVC_STATUS TVC_SetTarSizeForHQA(BOOL enable)
{
	...
	TV_INFO("Tar size for HQA is (%s)", (enable?"on":"off"));
	return TVC_STATUS_OK;
}
#endif
...
TVC_STATUS TVC_CommitChanges(BOOL blocking)
{
    if (_tvcContext.srcFormatSizeDirty)
    {
        ...
        TV_INFO("check line: %d", _tvcContext.tvc_checkline_cnt);
...
void TVC_ConfigSize(unsigned int src_width, unsigned int src_height, unsigned int tar_width, unsigned int tar_height)
{
    ...
    TV_INFO("src %d, %d, tar %d, %d", src_width, src_height, tar_width, tar_height);
