...
#define TV_DBG(fmt, arg...)      xlog_printk( ANDROID_LOG_DEBUG, "TV/TVR", "[DBG]: %s() "fmt"\n", __FUNCTION__, ##arg)
#define TV_INFO(fmt, arg...)     xlog_printk( ANDROID_LOG_INFO, "TV/TVR", "[INFO]: %s() "fmt"\n", __FUNCTION__, ##arg)
#define TV_WARN(fmt, arg...)     xlog_printk( ANDROID_LOG_WARN, "TV/TVR", "[WARN]: %s() "fmt"\n", __FUNCTION__, ##arg)
#define TV_ERROR(fmt, arg...)    xlog_printk( ANDROID_LOG_ERROR, "TV/TVR", "[ERR]: %s(): %s@%d "fmt"\n", __FUNCTION__, __FILE__,__LINE__,##arg)
#define TV_FATAL(fmt, arg...)    xlog_printk( ANDROID_LOG_FATAL, "TV/TVR", "[FATAL]: %s(): %s@%d "fmt"\n", __FUNCTION__, __FILE__,__LINE__,##arg)
...
static BOOL _alloc_and_set_sram(const TVR_PARAM *param)
{
    ...
    {
        TV_WARN("TVR SYSRAM already been set, but set again");
        return TRUE;
    }
    ...
    TV_INFO("Allocate internal SRAM, main_size: 0x%x, sub_size: 0x%x",
           main_size, sub_size);
    ...
    if (0 == main_buf_addr) {
        TV_ERROR("allocate internal SRAM failed");
        return FALSE;
    }
    ...
    // descriptor mode
    {
        ...
        for (i = 0; i < param->dstBufNum; ++ i) {
            ...
            while (0 == TVR_REG->QUEUE_WSTA.OK) {
                ++ timeout;
                if (timeout > 100000) {
                    TV_ERROR("QUEUE_DATA timeout dst addr");
                    return FALSE;
                }
            }
            OUTREG32(&TVR_REG->QUEUE_DATA, param->dstBufAddr[i] + dstBufOffset);
            ...
            while (0 == TVR_REG->QUEUE_WSTA.OK) {
                ++ timeout;
                if (timeout > 100000) {
                    TV_ERROR("QUEUE_DATA timeout src w/h");
                    return FALSE;
                }
            }
            ...
            while (0 == TVR_REG->QUEUE_WSTA.OK) {
                ++ timeout;
                if (timeout > 100000) {
                    TV_ERROR("QUEUE_DATA timeout src roi w/h");
                    return FALSE;
                }
            }
            ...
            while (0 == TVR_REG->QUEUE_WSTA.OK) {
                ++ timeout;
                if (timeout > 100000) {
                    TV_ERROR("QUEUE_DATA timeout src roi x/y");
                    return FALSE;
                }
            }
            ...
            while (0 == TVR_REG->QUEUE_WSTA.OK) {
                ++ timeout;
                if (timeout > 100000) {
                    TV_ERROR("QUEUE_DATA timeout dst with in byte");
                    return FALSE;
                }
            }
            ...
            while (0 == TVR_REG->QUEUE_WSTA.OK) {
                ++ timeout;
                if (timeout > 100000) {
                    TV_ERROR("QUEUE_DATA timeout con");
                    return FALSE;
                }
            }
...
static irqreturn_t _TVR_InterruptHandler(int irq, void *dev_id)
{
   ...
   // disable_irq(MT6575_TV_ROT_IRQ_ID);
    ...
    if (flag.FLAG1) {
        TV_ERROR("FLAG1: SW configuration error");
        clr.FLAG1_CLR = 1;
    }
    if (flag.FLAG5) {
        TV_ERROR("FLAG5: SW configuration error");
        clr.FLAG5_CLR = 1;
    }

    OUTREG32(&TVR_REG->IRQ_FLAG_CLR, AS_UINT32(&clr));
    //mt6575_irq_unmask(MT6575_TV_ROT_IRQ_ID);
   // enable_irq(MT6575_TV_ROT_IRQ_ID);
...
TVR_STATUS TVR_Init(void)
{
    ...
    {
        TV_ERROR("fail to request TVR irq");
...
{
    if (!s_isSramAllocated) {
        TV_ERROR("Working SRAM is not allocated!!");
        return TVR_STATUS_ERROR;
    }
...
bool TVR_EnqueueBuffer(UINT32 addr, UINT32* pAddrPrev)
{
    ...
    {
        TV_WARN("TVR has not been enabled");
        return true;
    }
