...
#define TV_DBG(fmt, arg...)      xlog_printk( ANDROID_LOG_DEBUG, "TV/TVE", "[DBG]: %s() "fmt"\n", __FUNCTION__, ##arg)
#define TV_INFO(fmt, arg...)     xlog_printk( ANDROID_LOG_INFO, "TV/TVE", "[INFO]: %s() "fmt"\n", __FUNCTION__, ##arg)
#define TV_WARN(fmt, arg...)     xlog_printk( ANDROID_LOG_WARN, "TV/TVE", "[WARN]: %s() "fmt"\n", __FUNCTION__, ##arg)
#define TV_ERROR(fmt, arg...)    xlog_printk( ANDROID_LOG_ERROR, "TV/TVE", "[ERR]: %s(): %s@%d "fmt"\n", __FUNCTION__, __FILE__,__LINE__,##arg)
#define TV_FATAL(fmt, arg...)    xlog_printk( ANDROID_LOG_FATAL, "TV/TVE", "[FATAL]: %s(): %s@%d "fmt"\n", __FUNCTION__, __FILE__,__LINE__,##arg)
...
#if 0
static irqreturn_t _TVE_InterruptHandler(int irq, void *dev_id)
{
    return IRQ_HANDLED;
}
#endif
...
TVE_STATUS TVE_Enable(void)
{
    ...
    */
    ...
    OUTREG32(0xF0007600,0x902);//Enable TV DAC
    //OUTREG32(0xF000780c,0x100);
