...
#define LCD_MASKREG32(addr, mask, data)	\
	...
	extern struct fb_overlay_buffer_list *overlay_buffer_head;
#endif
...
UINT32 const NLI_ARB_CS = 0xf100d014;
UINT32 const DPI_PAD_CON = 0xf2080900;

UINT32 const INFRA_DRV5 = 0xF0001814;
UINT32 const INFRA_DRV6 = 0xF0001818;
UINT32 const INFRA_DRV12 = 0xF0001830;
// ---------------------------------------------------------------------------
//  Local Functions
// ---------------------------------------------------------------------------
...
//	mt65xx_irq_unmask(MT6575_LCD_IRQ_ID);
//	enable_irq(MT6575_LCD_IRQ_ID);
...
#if defined(MTK_M4U_SUPPORT)
BOOL LCD_SearchMVA(UINT32 mva)
{
	BOOL ret = TRUE;
	struct fb_overlay_buffer_list *c;
	c = overlay_buffer_head->next;
	if(c == NULL){
		printk("[LCD Warning] mva = 0x%x, buffer list is NULL\n",mva);
		return FALSE;
	}
	while(mva != c->src_mva){
		c = c->next;
		if(!c){
			printk("[LCD Warning] mva = 0x%x,Can't find matched VA in buffer list\n",mva);
			ret = FALSE;
			break;
		}
	}
...
void LCD_ProtectTranslationFault(void)
{
	int i = 0;
	for(i=0;i<FB_LAYER;i++){
		if(LCD_IsLayerEnable((LCD_LAYER_ID)i)){
			if(!LCD_SearchMVA(LCD_REG->LAYER[(LCD_LAYER_ID)i].ADDRESS)){
