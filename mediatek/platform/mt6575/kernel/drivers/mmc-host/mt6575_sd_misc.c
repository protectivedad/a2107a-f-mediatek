...
#ifdef MTK_EMMC_SUPPORT
static int mt6575_mmc_get_disk_info(struct mbr_part_info* mpi, unsigned char* name)
{
    int i = 0;
...
static int mt6575_mmc_erase_func(unsigned int start, unsigned int size)
{
	struct msdc_host *host;
...
int mt6575_mmc_erase_partition(unsigned char* name)
{
#ifdef MTK_EMMC_SUPPORT
    ...
    if (('u' == *name && 's' == *(name + 1) && 'r' == *(name + 2) && 'd' == *(name + 3) && 
        ...
        l_ret = mt6575_mmc_get_disk_info(&mbr_part, name);


        if (l_ret == 0){
            /* do erase */
            printk("erase %s start sector: 0x%x size: 0x%x \n", mbr_part.part_name, mbr_part.start_sector, mbr_part.nr_sects);
            mt6575_mmc_erase_func(mbr_part.start_sector, mbr_part.nr_sects);
        }
    }
...
static int mt6575_mmc_erase_partition_wrap(struct msdc_ioctl* msdc_ctl)
{
    ...
    return mt6575_mmc_erase_partition(name);
}
...
static long simple_mt6575_sd_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
    ...
    switch (msdc_ctl->opcode){
        ...
        case MSDC_ERASE_PARTITION:
            msdc_ctl->result = mt6575_mmc_erase_partition_wrap(msdc_ctl);
            break;
        default:
