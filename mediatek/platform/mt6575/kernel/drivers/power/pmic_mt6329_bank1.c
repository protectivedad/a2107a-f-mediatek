...
/**********************************************************
  ...
  *
  *********************************************************/
extern int mt6575_i2c_polling_read(int port, unsigned char addr, unsigned char *buffer, int len);
extern int mt6575_i2c_polling_write(int port, unsigned char addr, unsigned char *buffer, int len);
...
else
{
	...
	buffer = RegNum;	    
	result_tmp = mt6575_i2c_polling_writeread(2,chip_slave_address,&buffer,1,1);
...
else
{
	...
	result_tmp = mt6575_i2c_polling_write(2,chip_slave_address,write_data,2);
...
void mt65xx_leds_turn_off(void)
{
...
static int pmic_thread_kthread(void *x)
{
    ...
    while (1) {           
	    ...
	    if(!upmu_get_PCHR_CHRDET() && is_first)   //Just once time
		{
			mt65xx_leds_turn_off();
			is_first = false;
			xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[pmic_thread_kthread] Turn Off Nleds\r\n");
		}
