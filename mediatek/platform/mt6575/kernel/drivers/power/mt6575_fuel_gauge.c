 ...
 * Description:
 * ------------
 *   This Module defines functions of 6575 fuel gauge
 *
 * Author:
 * -------
 * -------
...
{  
    ...
    if(en == KAL_TRUE)
    {
	    ...
	    xlog_printk(ANDROID_LOG_INFO, "Power/Battery", "[MTKFG_PLL_Control] MTKFG_PLL_Control ---ON \r\n"); 
    }
    else
    {
	    xlog_printk(ANDROID_LOG_INFO, "Power/Battery", "[MTKFG_PLL_Control] MTKFG_PLL_Control ---OFF \r\n");
    }
...
//// platform_driver API 
///////////////////////////////////////////////////////////////////////////////////////////
static int mt6575_fgadc_probe(struct platform_device *dev)	
{
...
#else
	xlog_printk(ANDROID_LOG_INFO, "Power/Battery", "[FGADC] MT6575 FGADC driver probe!! \n" );
...
static int mt6575_fgadc_remove(struct platform_device *dev)	
{
	xlog_printk(ANDROID_LOG_INFO, "Power/Battery", "[FGADC] MT6575 FGADC driver remove!! \n" );

	return 0;
}

static void mt6575_fgadc_shutdown(struct platform_device *dev)	
{
	xlog_printk(ANDROID_LOG_INFO, "Power/Battery", "[FGADC] MT6575 FGADC driver shutdown!! \n" );		
}
...
static int mt6575_fgadc_suspend(struct platform_device *dev, pm_message_t state)	
{

#if defined(CONFIG_POWER_EXT)
//#if 0
	xlog_printk(ANDROID_LOG_INFO, "Power/Battery", "[FGADC_suspend] EVB !!\n");
	...
	xlog_printk(ANDROID_LOG_INFO, "Power/Battery", "[FGADC_suspend] TODO !!\n");

	#if 0
	xlog_printk(ANDROID_LOG_INFO, "Power/Battery", "[FGADC_suspend] MT6573 FGADC driver suspend!!\n");
	...
	xlog_printk(ANDROID_LOG_INFO, "Power/Battery", "[FGADC_suspend] gRTC_time_suspend=%d, gFG_capacity_before=%d\n", 
...
static int mt6575_fgadc_resume(struct platform_device *dev)
{
#if defined(CONFIG_POWER_EXT)
//#if 0
	xlog_printk(ANDROID_LOG_INFO, "Power/Battery", "[FGADC_RESUME] EVB !!\n");

#else

	xlog_printk(ANDROID_LOG_INFO, "Power/Battery", "[FGADC_RESUME] TODO !!\n");
	...
	{
		xlog_printk(ANDROID_LOG_INFO, "Power/Battery", "[FGADC_RESUME] MT6573 FGADC driver resume, gFG_DOD0=%d!!\n", gFG_DOD0);
		...
		xlog_printk(ANDROID_LOG_INFO, "Power/Battery", "[FGADC_RESUME] FGADC_Reset some SW_Parameter \r\n");
		gFG_columb = 0;
		gFG_SW_CoulombCounter = 0;
		Temp_Reg = 0x18F0;
		...
		{	
			...
			xlog_printk(ANDROID_LOG_INFO, "Power/Battery", "[FGADC_RESUME] %d,%d,%d,%d\n", gFG_current, gFG_voltage, gFG_capacity_by_v, bat_volt_check_point);			
			...
			xlog_printk(ANDROID_LOG_INFO, "Power/Battery", "[FGADC_RESUME] Sleep Time(%d) > %d, gFG_capacity_by_v=%d, bat_volt_check_point=%d, gFG_DOD0=%d, gFG_DOD1=%d, gFG_percent_resume=%d\n", 
				gRTC_time_offset, gFG_RTC_time_MAX, gFG_capacity_by_v, bat_volt_check_point, gFG_DOD0, gFG_DOD1, gFG_percent_resume);
		}
		else
		{
			xlog_printk(ANDROID_LOG_INFO, "Power/Battery", "[FGADC_RESUME] Need update DOD0(%d) by bat_volt_check_point(%d)\n", 
			...
			xlog_printk(ANDROID_LOG_INFO, "Power/Battery", "[FGADC_RESUME] Sleep Time(%d) <= %d, gFG_capacity_by_v=%d, bat_volt_check_point=%d, gFG_DOD0=%d, gFG_DOD1=%d\n", 
				gRTC_time_offset, gFG_RTC_time_MAX, gFG_capacity_by_v, bat_volt_check_point, gFG_DOD0, gFG_DOD1);
		}

		xlog_printk(ANDROID_LOG_INFO, "Power/Battery", "[FGADC_RESUME] gRTC_time_suspend=%d, gRTC_time_resume=%d, gFG_capacity_before=%d, gFG_capacity_after=%d, temp_RTC=%d, temp_FG=%d\n", 
			gRTC_time_suspend, gRTC_time_resume, gFG_capacity_before, gFG_capacity_after, temp_RTC, temp_FG);
		
		xlog_printk(ANDROID_LOG_INFO, "Power/Battery", "[FGADC_RESUME] %d,%d,%d,%d,%d,gFG_temp=%d,gFG_DOD1_return=%d,val_car=%d\n", 
...
struct platform_device MT6575_fgadc_device = {
		.name				= "mt6575-fgadc",
		.id					= -1,
};

static struct platform_driver mt6575_fgadc_driver = {
	.probe		= mt6575_fgadc_probe,
	.remove		= mt6575_fgadc_remove,
	.shutdown	= mt6575_fgadc_shutdown,
	//#ifdef CONFIG_PM
	.suspend	= mt6575_fgadc_suspend,
	.resume		= mt6575_fgadc_resume,
	//#endif
	.driver         = {
        .name = "mt6575-fgadc",
    },
...
static int __init mt6575_fgadc_init(void)
{
	int ret;
	
	ret = platform_device_register(&MT6575_fgadc_device);
	if (ret) {
		xlog_printk(ANDROID_LOG_INFO, "Power/Battery", "****[mt6575_fgadc_driver] Unable to device register(%d)\n", ret);
		return ret;
	}
	
	ret = platform_driver_register(&mt6575_fgadc_driver);
	if (ret) {
		xlog_printk(ANDROID_LOG_INFO, "Power/Battery", "****[mt6575_fgadc_driver] Unable to register driver (%d)\n", ret);
		return ret;
	}

	xlog_printk(ANDROID_LOG_INFO, "Power/Battery", "****[mt6575_fgadc_driver] Initialization : DONE \n");

	return 0;

}

static void __exit mt6575_fgadc_exit (void)
{
}

module_init(mt6575_fgadc_init);
module_exit(mt6575_fgadc_exit);

MODULE_AUTHOR("James Lo");
MODULE_DESCRIPTION("MT6575 FGADC Device Driver");
MODULE_LICENSE("GPL");
