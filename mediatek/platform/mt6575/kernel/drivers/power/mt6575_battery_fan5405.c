...
PMU_STATUS BAT_BatteryFullAction(void)
{
	...
	if(gFG_can_reset_flag == 1)
	{
        ...
        {
            if (Enable_BATDRV_LOG >= 1) {
    			xlog_printk(ANDROID_LOG_INFO, "Power/Battery", "[BATTERY] VBAT<4150mV. Don't FGADC_Reset_SW_Parameter.\n", BMT_status.bat_vol);
    		}
        }
	}
	g_Calibration_FG = 0;
...
{
	if (Enable_BATDRV_LOG == 1) {
		printk("[MT6575 BAT_probe] ChargerHwInit\n" );
	}
...
int BAT_CheckBatteryStatus_fan5405(void)
{
    ...
    if (Enable_BATDRV_LOG >= 1) {
    	printk("[BATTERY:ADC:fan5405] VCHR:%ld BAT_SENSE:%ld I_SENSE:%ld TBAT:%d \n", 
			BMT_status.charger_vol, BMT_status.ADC_BAT_SENSE, BMT_status.ADC_I_SENSE, BMT_status.temperature);
    }
	...
	if (Enable_BATDRV_LOG >= 1) {
    	printk("[BATTERY:AVG:fan5405] BatTemp:%d Vbat:%ld VBatSen:%ld SOC:%ld ChrDet:%d Vchrin:%ld Icharging:%ld ChrType:%d USBstate:%d\r\n", 
    ...
    if (Enable_BATDRV_LOG >= 1) {		
		printk("[BATTERY:FG:fan5405] %d,%ld,%ld,%ld,%d,%ld,%ld,%d,%d,%d,%d\r\n", 
       	BMT_status.temperature ,BMT_status.bat_vol, BMT_status.ADC_BAT_SENSE, BMT_status.SOC, 
...
void BatThread_XGPTConfig(void)
{
	...
	xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery", "******** MT6575 battery : BatThread_XGPTConfig !********\n" );
