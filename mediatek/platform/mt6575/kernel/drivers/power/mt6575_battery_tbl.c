 ...
 * Description:
 * ------------
 *   This Module defines functions of mt6575 Battery charging algorithm
...
kal_bool upmu_is_chr_det(void)
{
	...
	{
#ifdef MTK_NCP1851_SUPPORT
            return KAL_TRUE; //tablet do not support OTG
#else
...
static struct proc_dir_entry *proc_entry_ctrl;
...
static int mt6575_ac_get_property(struct power_supply *psy,
	enum power_supply_property psp,
	union power_supply_propval *val)
{
    int ret = 0;
    struct mt6575_ac_data *data = container_of(psy, struct mt6575_ac_data, psy);
...
static int mt6575_usb_get_property(struct power_supply *psy,
	enum power_supply_property psp,
	union power_supply_propval *val)
{
    int ret = 0;
    struct mt6575_usb_data *data = container_of(psy, struct mt6575_usb_data, psy);
...
static int mt6575_battery_get_property(struct power_supply *psy,
	enum power_supply_property psp,
	union power_supply_propval *val)
{
    int ret = 0;
    struct mt6575_battery_data *data = container_of(psy, struct mt6575_battery_data, psy);
...
/* mt6575_ac_data initialization */
static struct mt6575_ac_data mt6575_ac_main = {
    .psy = {
	.name = "ac",
	.type = POWER_SUPPLY_TYPE_MAINS,
	.properties = mt6575_ac_props,
	.num_properties = ARRAY_SIZE(mt6575_ac_props),
	.get_property = mt6575_ac_get_property,
    },
    .AC_ONLINE = 0,
};

/* mt6575_usb_data initialization */
static struct mt6575_usb_data mt6575_usb_main = {
    .psy = {
	.name = "usb",
	.type = POWER_SUPPLY_TYPE_USB,
	.properties = mt6575_usb_props,
	.num_properties = ARRAY_SIZE(mt6575_usb_props),
	.get_property = mt6575_usb_get_property,
    },
    .USB_ONLINE = 0,
};

/* mt6575_battery_data initialization */
static struct mt6575_battery_data mt6575_battery_main = {
    .psy = {
	.name = "battery",
	.type = POWER_SUPPLY_TYPE_BATTERY,
	.properties = mt6575_battery_props,
	.num_properties = ARRAY_SIZE(mt6575_battery_props),
	.get_property = mt6575_battery_get_property,
    },
...
#if defined(CONFIG_POWER_EXT)
#else
static void mt6575_ac_update(struct mt6575_ac_data *ac_data)
{
...
static void mt6575_usb_update(struct mt6575_usb_data *usb_data)
{
...
static void mt6575_battery_update(struct mt6575_battery_data *bat_data)
{
...
static void mt6575_battery_update_power_down(struct mt6575_battery_data *bat_data)
{
    struct power_supply *bat_psy = &bat_data->psy;

    bat_data->BAT_CAPACITY = 0;

	xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery", "[Battery] mt6575_battery_update_power_down\r\n");
...
{
	if (Enable_BATDRV_LOG == 1) {
		xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery", "[MT6575 BAT_probe] ChargerHwInit\n" );
	}
...
int BAT_CheckBatteryStatus_ncp1851(void)
{
    ...
    if (Enable_BATDRV_LOG == 1) {
        xlog_printk(ANDROID_LOG_INFO, "Power/Battery", "[BATTERY:ADC:ncp1851] VCHR:%ld BAT_SENSE:%ld I_SENSE:%ld TBAT:%ld TBAT_vol:%ld \n",
...
void BAT_thread(void)
{
    ...
    if(g_battery_thermal_throttling_flag==1)
    {
        ...
        {
            #if defined(CONFIG_POWER_EXT)
	        xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery", "[BATTERY] CONFIG_POWER_EXT, no update mt6575_battery_update_power_down.\n");
	        #else
            xlog_printk(ANDROID_LOG_INFO, "Power/Battery", "[Battery] Tbat(%d)>=60, system need power down.\n", BMT_status.temperature);
			mt6575_battery_update_power_down(&mt6575_battery_main);
            if( upmu_is_chr_det() == KAL_TRUE )
            {
    ...
    mt6575_ac_update(&mt6575_ac_main);
    mt6575_usb_update(&mt6575_usb_main);
    mt6575_battery_update(&mt6575_battery_main);
	}
	else
	{
...
void BAT_thread_ncp1851(void)
{
    ...
    if (Enable_BATDRV_LOG == 1) {
        xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery", "[BATTERY_TOP] LOG. %d,%d,%d,%d----------------------------\n",
    ...
    if(get_pmic_flag == 0)
    {
        ...
        {
            xlog_printk(ANDROID_LOG_INFO, "Power/Battery", "[Battery] Get PMIC version > E1\n");
        }
    ...
    if( upmu_is_chr_det() == KAL_TRUE )
    {
        ...
        {
            CHR_Type_num = mt_charger_type_detection();
            xlog_printk(ANDROID_LOG_INFO, "Power/Battery", "[BAT_thread] CHR_Type_num = %d\r\n", CHR_Type_num);
            BMT_status.charger_type = CHR_Type_num;
            if( (CHR_Type_num==STANDARD_HOST) || (CHR_Type_num==CHARGING_HOST) )
            {
                mt_usb_connect();
    ...
    {
         xlog_printk(ANDROID_LOG_INFO, "Power/Battery", "[BATTERY] gFG_booting_counter_I_FLAG!=2 (%d)\r\n", gFG_booting_counter_I_FLAG);
    }
    ...
    if(BAT_status == PMU_STATUS_FAIL || g_Battery_Fail)
    {
        ...
        if (Enable_BATDRV_LOG == 1)
        {
            xlog_printk(ANDROID_LOG_INFO, "Power/Battery", "[BATTERY:ncp1851] PMU_STATUS_FAIL \n");
        }
    }
    /* Battery Full */
    /* HW charging done, real stop charging */
...
void PrechargeCheckStatus(void)
{
    ...
    int bat_temp_vol, bat_temp, charger_vol;
    int bat_vol_chk = 0;
    kal_uint32 vfet_status, ncp1851_status;
    struct power_supply *bat_psy;
    struct mt6575_battery_data *bat_data;
    int chr_err_cnt = 0;
    ...
    while(1)
    {
        if(g_pmic_init_for_ncp1851 == 1) //make sure mt6575_battery_probe register finished
	     break;
        else
            msleep(1000);
    }
...
//        if((bat_vol >= SYSTEM_OFF_VOLTAGE + 100) && (vfet_status == 1)) //+100 may need furthur tuning
        if(vfet_status == 1)
        {
            g_ocv_lookup_done = 0;
            pchr_turn_off_charging_ncp1851();
...
#if 0
        else if(bat_vol >= SYSTEM_OFF_VOLTAGE + 100)
        {
            gFG_capacity = 1;
            bat_volt_check_point = 1;
            g_ocv_lookup_done = 1;
    ...
    while(1)
    {
        if( upmu_is_chr_det() == KAL_TRUE )
        {
            ...
            else
            {
                ...
                if(BMT_status.bat_charging_state == CHR_ERROR)
                {
                    chr_err_cnt = 120; // 120 * 5 = 600secs = 10 mins
                }
                else
                {
                    if(chr_err_cnt > 0)
...
int bat_thread_kthread(void *x)
{
    ...
    while (1) {
		...
		if (Enable_BATDRV_LOG == 1) {
			xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery", "******** MT6575 battery : bat_thread_kthread : 1 ********\n" );
		}

		wait_event(bat_thread_wq, bat_thread_timeout);

		if (Enable_BATDRV_LOG == 1) {
			xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery", "******** MT6575 battery : bat_thread_kthread : 2 ********\n" );
		}
...
{
	if (Enable_BATDRV_LOG == 1) {
		xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery", "******** MT6575 battery : bat_thread_wakeup : 1 ********\n" );
	}
	bat_thread_timeout = 1;
	wake_up(&bat_thread_wq);
	if (Enable_BATDRV_LOG == 1) {
		xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery", "******** MT6575 battery : bat_thread_wakeup : 2 ********\n" );
	}
}
...
void BatThread_XGPTConfig(void)
{
	...
	xlog_printk(ANDROID_LOG_INFO, "Power/Battery", "******** MT6575 battery : BatThread_XGPTConfig !********\n" );
...
static int mt6575_battery_probe(struct platform_device *dev)
{
    ...
    xlog_printk(ANDROID_LOG_INFO, "Power/Battery", "******** MT6575 battery driver probe!! ********\n" );
	...
	class_dev = (struct class_device *)device_create(adc_cali_class,
												   ...
												   NULL,
												   ADC_CALI_DEVNAME);
	xlog_printk(ANDROID_LOG_INFO, "Power/Battery", "[MT6575 BAT_probe] NVRAM prepare : done !!\n ");

	/* Integrate with Android Battery Service */
    ret = power_supply_register(&(dev->dev), &mt6575_ac_main.psy);
    if (ret)
    {
	xlog_printk(ANDROID_LOG_INFO, "Power/Battery", "[MT6575 BAT_probe] power_supply_register AC Fail !!\n");
	return ret;
    }
    xlog_printk(ANDROID_LOG_INFO, "Power/Battery", "[MT6575 BAT_probe] power_supply_register AC Success !!\n");

    ret = power_supply_register(&(dev->dev), &mt6575_usb_main.psy);
    if (ret)
    {
	xlog_printk(ANDROID_LOG_INFO, "Power/Battery", "[MT6575 BAT_probe] power_supply_register USB Fail !!\n");
	return ret;
    }
    xlog_printk(ANDROID_LOG_INFO, "Power/Battery", "[MT6575 BAT_probe] power_supply_register USB Success !!\n");

    ret = power_supply_register(&(dev->dev), &mt6575_battery_main.psy);
    if (ret)
    {
	xlog_printk(ANDROID_LOG_INFO, "Power/Battery", "[MT6575 BAT_probe] power_supply_register Battery Fail !!\n");
	return ret;
    }
    xlog_printk(ANDROID_LOG_INFO, "Power/Battery", "[MT6575 BAT_probe] power_supply_register Battery Success !!\n");
...
static int mt6575_battery_remove(struct platform_device *dev)
{
    xlog_printk(ANDROID_LOG_INFO, "Power/Battery", "******** MT6575 battery driver remove!! ********\n" );

    return 0;
}

static void mt6575_battery_shutdown(struct platform_device *dev)
{
    xlog_printk(ANDROID_LOG_INFO, "Power/Battery", "******** MT6575 battery driver shutdown!! ********\n" );

}

static int mt6575_battery_suspend(struct platform_device *dev, pm_message_t state)
{
    xlog_printk(ANDROID_LOG_INFO, "Power/Battery", "******** MT6575 battery driver suspend!! ********\n" );

    return 0;
}

static int mt6575_battery_resume(struct platform_device *dev)
{
    xlog_printk(ANDROID_LOG_INFO, "Power/Battery", "******** MT6575 battery driver resume!! ********\n" );
...
struct platform_device MT6575_battery_device = {
    .name   = "mt6575-battery",
    .id	    = -1,
};

static struct platform_driver mt6575_battery_driver = {
    .probe		= mt6575_battery_probe,
    .remove		= mt6575_battery_remove,
    .shutdown	= mt6575_battery_shutdown,
    //#ifdef CONFIG_PM
    .suspend	= mt6575_battery_suspend,
    .resume		= mt6575_battery_resume,
    //#endif
    .driver     = {
        .name = "mt6575-battery",
    },
...
static int __init mt6575_battery_init(void)
{
    int ret;

    ret = platform_device_register(&MT6575_battery_device);
    if (ret) {
	xlog_printk(ANDROID_LOG_INFO, "Power/Battery", "****[mt6575_battery_driver] Unable to device register(%d)\n", ret);
	return ret;
    }
    ret = platform_driver_register(&mt6575_battery_driver);
    if (ret) {
	xlog_printk(ANDROID_LOG_INFO, "Power/Battery", "****[mt6575_battery_driver] Unable to register driver (%d)\n", ret);
	return ret;
    }
    ...
    xlog_printk(ANDROID_LOG_INFO, "Power/Battery", "****[mt6575_battery_driver] Initialization : DONE !!\n");

    return 0;
}

static void __exit mt6575_battery_exit (void)
{
}

module_init(mt6575_battery_init);
module_exit(mt6575_battery_exit);

MODULE_AUTHOR("James Lo");
MODULE_DESCRIPTION("MT6575 Battery Device Driver");
MODULE_LICENSE("GPL");
