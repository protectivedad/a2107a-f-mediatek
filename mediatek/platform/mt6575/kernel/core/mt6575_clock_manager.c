...
{
    xlog_printk(ANDROID_LOG_INFO, TAG, "[%s]: %s = 0x%08x\n", __func__, mt65xx_category_name[category], 
...
{
    xlog_printk(ANDROID_LOG_INFO, TAG, "[%s]: %s%s = 0x%04x\n", __func__, mt65xx_pll_name[id], 
...
int get_state(int type, int id, int mode)
{
    ...
    default:
        xlog_printk(ANDROID_LOG_WARN, TAG, "[%s]: type = %d\n", __func__, type);
        return -1;
    }
}
...
static int should_power_on_subsys(int subsys_id) {
    ...
    default:
        xlog_printk(ANDROID_LOG_WARN, TAG, "[%s]bad subsys id %d\n", __func__, subsys_id);
        BUG();
    }
}
...
static int should_power_on_mmsys_clksrc(void) {
    return !clk_mgr.mmsys_state;
}
...
static int __request_mmsys_clksrc(int opcode);
static void mm_power_prepare(int subsys_id, char *usr, int usr_id)
{
    if (should_power_on_mmsys_clksrc()) {
        xlog_printk(ANDROID_LOG_INFO, TAG, "[%s]: enable mmsys clksrc for %s %d\n", __func__, usr, usr_id);
        __request_mmsys_clksrc(1);

		xlog_printk(ANDROID_LOG_INFO, TAG, "[%s]: set MM way_en\n", __func__);
		DRV_SetReg32(TOPAXI_SI4_CTL, 0x1 << 5);
    }

    if (subsys_id) {
        if ((subsys_id == MT65XX_SUBSYS_MM1) && should_power_on_subsys(MT65XX_SUBSYS_MM1)) {
            xlog_printk(ANDROID_LOG_INFO, TAG, "[%s]: subsys MM1 is OFF now, enable it first for %s %d\n", 
                    __func__, usr, usr_id);
            enable_subsys(MT65XX_SUBSYS_MM1, "clkmgr");
        } else if (subsys_id == MT65XX_SUBSYS_MM2) {
            if (should_power_on_subsys(MT65XX_SUBSYS_MM2)) {
                xlog_printk(ANDROID_LOG_INFO, TAG, "[%s]: subsys MM2 is OFF now, enable it first for %s %d\n", 
                        __func__, usr, usr_id);
                enable_subsys(MT65XX_SUBSYS_MM2, "clkmgr");
            } else {
#ifdef MM1_TRIGGER_MM2
                if (pdn_mm2_pending)
                    pdn_mm2_pending = 0;
...
static void mm_power_finish(int subsys_id, char *usr, int usr_id) {
    if (subsys_id == MT65XX_SUBSYS_MM1 && could_power_off_subsys(MT65XX_SUBSYS_MM1)) {
        xlog_printk(ANDROID_LOG_INFO, TAG, "[%s]: disable subsys MM1 for %s %d\n", __func__, usr, usr_id);
        disable_subsys(MT65XX_SUBSYS_MM1, "clkmgr");
#ifdef MM1_TRIGGER_MM2
        if (pdn_mm2_pending) {
            xlog_printk(ANDROID_LOG_INFO, TAG, "[%s]: disable subsys MM2 triggered by MM1\n", __func__);
            disable_subsys(MT65XX_SUBSYS_MM2, "clkmgr");
            pdn_mm2_pending = 0;
        }
#endif
        ...
        if (clk_mgr.subsys_state & (0x1 << MT65XX_SUBSYS_MM1)) {
            if (!pdn_mm2_pending) {
                //xlog_printk(ANDROID_LOG_INFO, TAG, "[%s]: disable subsys MM2 for %s %d pending\n", __func__, usr, usr_id);
                pdn_mm2_pending = 1;
            }
        } else {
            xlog_printk(ANDROID_LOG_INFO, TAG, "[%s]: disable subsys MM2 for %s %d now\n", __func__, usr, usr_id);
            disable_subsys(MT65XX_SUBSYS_MM2, "clkmgr");
            pdn_mm2_pending = 0;
        }
#else
        xlog_printk(ANDROID_LOG_INFO, TAG, "[%s]: disable subsys MM2 for %s %d\n", __func__, usr, usr_id);
        disable_subsys(MT65XX_SUBSYS_MM2, "clkmgr");
#endif
    }

    if (clk_mgr.mmsys_state && !(clk_mgr.subsys_state & 0x30) && !(clk_mgr.clock_state[MT65XX_CLOCK_MMSYS2_PDN])) {
        xlog_printk(ANDROID_LOG_INFO, TAG, "[%s]: clr MM way_en\n", __func__);
		DRV_ClrReg32(TOPAXI_SI4_CTL, 0x1 << 5);

        xlog_printk(ANDROID_LOG_INFO, TAG, "[%s]: disable mmsys clksrc for %s %d\n", __func__, usr, usr_id);
        __request_mmsys_clksrc(0);
    }
}
...
static void clk_stat_locked(int category, int offset, char* mod_name, int opt)
{
    ...
    if (new_module) {
        ...
        if (!info) {
            xlog_printk(ANDROID_LOG_ERROR, TAG, "[%s]: malloc module_info for %s fail\n", 
                __func__, mod_name);
            return;
        } else {
...
static int enable_clock_internal(int category, unsigned int mask)
{
    ...
    if (partial_log_mask[category] & mask) {
        xlog_printk(ANDROID_LOG_INFO, TAG, "[%s]: <== UNgate 0x%x @ %s\n", __func__, mask,
    ...
    if (get_clock_state(category, mask, REG_STATE) == STATE_OFF) {
        xlog_printk(ANDROID_LOG_ERROR, TAG, "[%s]: enable 0x%x @ %s fail\n", __func__, mask, 
                mt65xx_category_name[category]);
        return 1;
    }
    ...
    if(category == MT65XX_CLOCK_MMSYS2_PDN && (mask & MFG_CHECK_MASK) && 
        ...
        if (partial_log_mask[category] & mask) {
            xlog_printk(ANDROID_LOG_INFO, TAG, "[%s]: set way_en for mfg\n", __func__);
        }
    ...
    if (category == MT65XX_CLOCK_MMSYS2_PDN && (mask & AUD_CORE_MASK) && 
        ...
        if (partial_log_mask[category] & mask) {
            xlog_printk(ANDROID_LOG_INFO, TAG, "[%s]: set way_en for audio\n", __func__);
        }
...
static int disable_clock_internal(int category, unsigned int mask)
{
    ...
    if (category == MT65XX_CLOCK_MMSYS2_PDN && (mask & MFG_CHECK_MASK) && 
            way_en[WAY_EN_IDX_MFG]) {
        if (partial_log_mask[category] & mask) {
            xlog_printk(ANDROID_LOG_INFO, TAG, "[%s]: clr way_en for mfg\n", __func__);
        }
    ...
    if (category == MT65XX_CLOCK_MMSYS2_PDN && (mask & AUD_CORE_MASK) &&
            way_en[WAY_EN_IDX_AUD]) {
        if (partial_log_mask[category] & mask) {
            xlog_printk(ANDROID_LOG_INFO, TAG, "[%s]: clr way_en for audio\n", __func__);
        }
    ...
    if (partial_log_mask[category] & mask) {
        xlog_printk(ANDROID_LOG_INFO, TAG, "[%s]: <== gate 0x%x @ %s\n", __func__, mask,
                mt65xx_category_name[category]);
        dump_clk_reg(category);
    }
    ...
    if (get_clock_state(category, mask, REG_STATE) == STATE_ON) {
        xlog_printk(ANDROID_LOG_ERROR, TAG, "[%s]: disable 0x%x @ %s fail\n", __func__, mask, 
                mt65xx_category_name[category]);
        return 1;
    }
...
#define WPLL_CLKSRC_MASK_ALL        (WPLL_61M_EN | WPLL_197M_EN | WPLL_EN)
...
void clksrc_select(int id, unsigned int selection)
{
    ...
    xlog_printk(ANDROID_LOG_INFO, TAG, "[%s] %d change clksrc: 0x%x -> 0x%x\n", 
...
#if 0
    xlog_printk(ANDROID_LOG_INFO, TAG, 
...
static void larb_backup(int larb_idx)
{
    ...
    //if (clkmgr_debug_mask & DEBUG_LARB) {
        xlog_printk(ANDROID_LOG_INFO, TAG, "[%s]: start to backup larb %d\n", __func__, larb_idx);
    //}
...
static void larb_restore(int larb_idx)
{
    ...
    //if (clkmgr_debug_mask & DEBUG_LARB) {
        xlog_printk(ANDROID_LOG_INFO, TAG, "[%s]: start to restore larb %d\n", __func__, larb_idx);
    //}
...
static void enable_larb_internal(int larb_idx, char* mod_name)
{
    ...
    for (i = larb_modules[larb_idx].start; i < (larb_modules[larb_idx].start + larb_modules[larb_idx].number); i++) {
        ...
        if (partial_log_mask[category] & larb_clocks[i].mask) {
            xlog_printk(ANDROID_LOG_INFO, TAG, "[%s]: before ++, clock[%d].count = %d\n",
    ...
    if (!larb_modules[larb_idx].dirty) {
        if (clkmgr_debug_mask & DEBUG_LARB) {
            xlog_printk(ANDROID_LOG_INFO, TAG, "[%s]: larb%d clock is 1st enabled\n", __func__, larb_idx);
        }
        restore = 1;
    }

    if ((clkmgr_debug_mask & DEBUG_LARB) && mask && mask != larb_modules[larb_idx].mask) {
        xlog_printk(ANDROID_LOG_INFO, TAG, "[%s]: larb%d mask: 0x%08x, mask: 0x%08x\n", 
    ...
    if (mask) {
        switch (larb_idx) {
            ...
            default:
                xlog_printk(ANDROID_LOG_WARN, TAG, "[%s]: larb_idx %d\n", __func__, larb_idx);
        }
    }
...
bool larb_is_on(int larb_idx)
{
	...
	switch (larb_idx) {
		...
		default:
			xlog_printk(ANDROID_LOG_INFO, TAG, "[%s]bad larb id %d\n", __func__, larb_idx);
			BUG_ON(1);
	}
...
int enable_larb(int larb_idx, char *mod_name)
{
    ...
    if (!initialized) {
        xlog_printk(ANDROID_LOG_WARN, TAG, "[%s]: clkmgr not initialized yet! [id=%d] [mod=%s]\n",
    ...
    if (INVALID_LARB_ID(larb_idx)) {
        xlog_printk(ANDROID_LOG_WARN, TAG, "[%s]: %s pass an invalid id [%d]\n",
    ...
    if (clkmgr_debug_mask & DEBUG_LARB) {
        xlog_printk(ANDROID_LOG_INFO, TAG, "[%s]: [id=%d] [mod=%s]\n", 
                __func__, larb_idx, mod_name);
    }
...
int enable_clock(enum mt65xx_clock_id id, char *mod_name)
{
    ...
    if (INVALID_CATEGORY_ID(category) || INVALID_CLOCK_ID(category, mask)) {
        xlog_printk(ANDROID_LOG_WARN, TAG, "[%s]: %s pass an invalid id [%d]: [%d][%d]\n",
    ...
    if ((clkmgr_debug_mask & DEBUG_CLOCK) || (partial_log_mask[category] & mask)) {
        xlog_printk(ANDROID_LOG_INFO, TAG, "[%s]: [id=%d] [category=%d] [offset=%d] [name=%s] [mod=%s]\n", 
    ...
    if (clk_mgr.clock[category][offset].count > 1) {
        ...
        xlog_printk(ANDROID_LOG_ERROR, TAG, "[%s]: after ++: clk_mgr.clock[%d].count = %d\n", 
                __func__, id, clk_mgr.clock[category][offset].count);
        spin_unlock_irqrestore(&clkmgr_lock, flags);
        return 1;
    }
    ...
    if (CATEGORY_AP1(category)) {
        ...
        if (wpll_clksrc_mask) {
            //xlog_printk(ANDROID_LOG_INFO, TAG, "[%s] enable wpll clksrc 0x%x for clock %d\n", __func__, wpll_clksrc_mask, id);
    ...
    if (CATEGORY_MM(category)) {
        ...
        if (!(nondepend_clock_mask[mm_category] & mask)) {
            ...
            if (unlikely(larb_clock_mask[mm_category] & mask)) {
                if (!larb_modules[larb_idx].dirty) {
                    if (clkmgr_debug_mask & DEBUG_LARB) {
                        xlog_printk(ANDROID_LOG_INFO, TAG, "[%s]: larb%d clock[%d] is 1st enabled\n", 
                ...
                if (clkmgr_debug_mask & DEBUG_LARB) {
                    xlog_printk(ANDROID_LOG_INFO, TAG, "[%s]: clock[%d] depends on larb %d\n", 
...
int disable_clock(enum mt65xx_clock_id id, char *mod_name)
{
    ...
    if (clk_mgr.clock[category][offset].count <= 0) {
        xlog_printk(ANDROID_LOG_WARN, TAG, "[%s]: clock[%d].count = %d, %s request to disable it\n", 
    ...
    if ((clkmgr_debug_mask & DEBUG_CLOCK) || 
            (partial_log_mask[category] & mask)) {
        xlog_printk(ANDROID_LOG_WARN, TAG, "[%s]: before --: clock[%d].count = %d\n", 
                __func__, id, clk_mgr.clock[category][offset].count);
    }
    ...
    if ((force_ungate_mask[category] & mask)) {
        xlog_printk(ANDROID_LOG_INFO, TAG, "[%s]: id = %d, it's a force ungated clock bit\n", __func__, id);

        if (CATEGORY_AP1(category)) {
            wpll_clksrc_mask = should_power_off_wpll_clksrc(mask);
            if (wpll_clksrc_mask) {
                //xlog_printk(ANDROID_LOG_INFO, TAG, "[%s] disable wpll clksrc 0x%x for clock %d\n", __func__, wpll_clksrc_mask, id);
    ...
    if (CATEGORY_MM(category)) {
        ...
        if (!(nondepend_clock_mask[mm_category] & mask)) {
            larb_idx = index_of(mm_category, mask); 
            //xlog_printk(ANDROID_LOG_INFO, TAG, "[%s]larb_idx = %d for clock %d\n", __func__, larb_idx, id);
            BUG_ON(larb_idx >= MT65XX_LARB_GROUP_COUNT);
...
{
    xlog_printk(ANDROID_LOG_INFO, TAG, "[%s] PLL_CON1    = 0x%04x\n", __func__, DRV_Reg16(PLL_CON1));
    xlog_printk(ANDROID_LOG_INFO, TAG, "[%s] MIPITX_CON0 = 0x%04x\n", __func__, DRV_Reg16(MIPI_TX_CON0));
    xlog_printk(ANDROID_LOG_INFO, TAG, "[%s] MIPITX_CON6 = 0x%04x\n", __func__, DRV_Reg16(MIPI_TX_CON6));
    xlog_printk(ANDROID_LOG_INFO, TAG, "[%s] MIPITX_CON9 = 0x%04x\n", __func__, DRV_Reg16(MIPI_TX_CON9));
    xlog_printk(ANDROID_LOG_INFO, TAG, "[%s] MIPIRX_CON1 = 0x%04x\n", __func__, DRV_Reg16(MIPI_RX_CON1));
    xlog_printk(ANDROID_LOG_INFO, TAG, "[%s] MIPIRX_CON4 = 0x%04x\n", __func__, DRV_Reg16(MIPI_RX_CON4));
    xlog_printk(ANDROID_LOG_INFO, TAG, "[%s] MIPIRX_CON5 = 0x%04x\n", __func__, DRV_Reg16(MIPI_RX_CON5));
    xlog_printk(ANDROID_LOG_INFO, TAG, "[%s] MIPI_CON    = 0x%08x\n", __func__, DRV_Reg32(MIPI_CON));
}
...
static void enable_mipi_op(int id, int tx_in_use, int rx_in_use)
{
    ...
    default:
        xlog_printk(ANDROID_LOG_INFO, TAG, "[%s]bad mipi id %d\n", __func__, id);
        BUG();
    }
...
int enable_mipi(enum mt65xx_mipi_id id, char *mod_name)
{
    ...
    if (clkmgr_debug_mask & DEBUG_MIPI) {
        xlog_printk(ANDROID_LOG_INFO, TAG, "[%s]: [id=%d] [name=%s] [mod=%s]\n", 
    ...
    if (clkmgr_debug_mask & DEBUG_MIPI) {
        xlog_printk(ANDROID_LOG_INFO, TAG, "[%s]: before ++: mipi[%d].count = %d\n", 
    ...
    if (clkmgr_debug_mask & DEBUG_MIPI) {
        xlog_printk(ANDROID_LOG_INFO, TAG, "[%s]: <== enable mipi %s\n", __func__, mipi[id].name);
        dump_mipi_regs();
    }
...
int disable_mipi(enum mt65xx_mipi_id id, char *mod_name)
{
    ...
    if (clkmgr_debug_mask & DEBUG_MIPI) {
        xlog_printk(ANDROID_LOG_INFO, TAG, "[%s]: <== disable mipi %s\n", __func__, mipi[id].name);
        dump_mipi_regs();
    }        
...
int enable_pll(enum mt65xx_pll_id id, char *mod_name)
{
    ...
    if (clkmgr_debug_mask & DEBUG_PLL) { 
        xlog_printk(ANDROID_LOG_INFO, TAG, "[%s]: before ++: pll[%d].count = %d\n", 
    ...
    if (clk_mgr.pll[id].count > 1) {
        ...
        xlog_printk(ANDROID_LOG_WARN, TAG, "[%s]: after ++: clk_mgr.pll[%d].count = %d\n", 
                __func__, id, clk_mgr.pll[id].count);
        spin_unlock_irqrestore(&clkmgr_lock, flags);
        return 1;
    }
    ...
    if (id == MT65XX_TVDDS) {
        xlog_printk(ANDROID_LOG_INFO, TAG, "[%s]: enable UPLL first before enable TVDDS\n", __func__);
        enable_pll(MT65XX_UPLL, "clkmgr");
    }

    enable_pll_op(id);

    if (get_pll_state(id, REG_STATE) == STATE_OFF) {
        xlog_printk(ANDROID_LOG_ERROR, TAG, "[%s]: enable %s fail\n", 
...
int disable_pll(enum mt65xx_pll_id id, char *mod_name)
{
    ...
    if (clk_mgr.pll[id].count <= 0) {
        xlog_printk(ANDROID_LOG_WARN, TAG, "[%s]: pll[%d].count = %d, %s request to disable it\n", 
    ...
    if (get_pll_state(id, REG_STATE) == STATE_ON) {
        xlog_printk(ANDROID_LOG_ERROR, TAG, "[%s]: disable %s fail\n", 
                __func__, clk_mgr.pll[id].name);
        spin_unlock_irqrestore(&clkmgr_lock, flags);
        return 1;
    }
    ...
    if (id == MT65XX_TVDDS) {
        xlog_printk(ANDROID_LOG_INFO, TAG, "[%s]: disable UPLL after disable TVDDS\n", __func__);
        disable_pll(MT65XX_UPLL, "clkmgr");
    }
...
int pll_fsel(enum mt65xx_pll_id id, unsigned int pll_value)
{
    ...
    if ((0x1 << id) & FIXED_FREQ_PLLS) {
        xlog_printk(ANDROID_LOG_WARN, TAG, "[%s]: %s center frequence fixed\n", 
                __func__, clk_mgr.pll[id].name);
        return 1;
    }
...
unsigned int freq_meter(int clk_sel, unsigned int delay)
{
    ...
    xlog_printk(ANDROID_LOG_INFO, TAG, "[%s] data = %d, freq = %d MHz\n", __func__, data, freq);    
...
static int __request_mmsys_clksrc(int opcode)
{
    ...
    if (unlikely(clk_mgr.mmsys_state == !!opcode)) {
        xlog_printk(ANDROID_LOG_INFO, TAG, "[%s]: opcode = %d, mmsys_state = %d, return directly\n",
    ...
    /* opcode: zero, release mmsys clksrc */
        ...
        if (!could_power_off_mmsys_clksrc()) {
            xlog_printk(ANDROID_LOG_WARN, TAG, "[%s]: mmsys clksrc is still used, so skip\n", __func__);
            spin_unlock_irqrestore(&clkmgr_lock, flags);
            return 1;
        }
...
int request_mmsys_clksrc(int opcode) {
    if (!initialized) {
        return 1;
    }
    
    return __request_mmsys_clksrc(opcode);
}
...
static void switch_mmsys_clksrc(void) 
{
    ...
    if (clkmgr_debug_mask & DEBUG_INIT) {
        freq_meter(0x11, 0);
        freq_meter(0x20, 0);
        xlog_printk(ANDROID_LOG_INFO, TAG, "[%s]: TOP_CKMUXSEL  = 0x%x\n", __func__, DRV_Reg32(TOP_CKMUXSEL));
        xlog_printk(ANDROID_LOG_INFO, TAG, "[%s]: TOP_CKDIV23   = 0x%x\n", __func__, DRV_Reg32(TOP_CKDIV23));
        xlog_printk(ANDROID_LOG_INFO, TAG, "[%s]: TOP_CKMUXSEL2 = 0x%x\n", __func__, DRV_Reg16(TOP_CKMUXSEL2));
    }
}
...
int enable_subsys(enum mt65xx_subsys_id id, char *mod_name)
{
    ...
    if (get_subsys_state(id, LOCAL_STATE) == STATE_ON) {
        if (clkmgr_debug_mask & DEBUG_SUBSYS) {
            xlog_printk(ANDROID_LOG_INFO, TAG, "[%s]: %s is already ON now\n", 
    ...
    if (id == MT65XX_SUBSYS_MM1) {
        xlog_printk(ANDROID_LOG_INFO, TAG, "[%s]: gate all MM1 clocks after power on MMSYS1\n", __func__);
        ...
        if (unlikely(get_chip_eco_ver() == CHIP_E1)) {
            xlog_printk(ANDROID_LOG_INFO, TAG, "[%s]: unmask M4U irq after power on MMSYS1\n", __func__);
            mt6575_irq_set_polarity(MT6575_SMI_LARB0_MMU_IRQ_ID, MT65xx_POLARITY_HIGH);
            mt6575_irq_set_polarity(MT6575_SMI_LARB1_MMU_IRQ_ID, MT65xx_POLARITY_HIGH);
            mt6575_irq_set_polarity(MT6575_SMI_LARB2_MMU_IRQ_ID, MT65xx_POLARITY_HIGH);
            enable_irq(MT6575_SMI_LARB0_MMU_IRQ_ID);    
            enable_irq(MT6575_SMI_LARB1_MMU_IRQ_ID);    
            enable_irq(MT6575_SMI_LARB2_MMU_IRQ_ID);
        }
    }
...
int disable_subsys(enum mt65xx_subsys_id id, char *mod_name)
{
    ...
    if (!could_power_off_subsys(id) && strcmp(mod_name, "subsys_test")) {
        xlog_printk(ANDROID_LOG_WARN, TAG, "[%s]: %s is still used, so skip\n", 
                __func__, clk_mgr.subsys[id].name);
        spin_unlock_irqrestore(&clkmgr_lock, flags);
        return 1;
    }
    ...
    if (get_subsys_state(id, LOCAL_STATE) == STATE_OFF) {
        if (clkmgr_debug_mask & DEBUG_SUBSYS) {
            xlog_printk(ANDROID_LOG_INFO, TAG, "[%s]: %s is already OFF now\n", 
                    __func__, clk_mgr.subsys[id].name);
        }
        spin_unlock_irqrestore(&clkmgr_lock, flags);
        return 0;
    ...
    if (id == MT65XX_SUBSYS_MM1) {
        xlog_printk(ANDROID_LOG_INFO, TAG, "[%s]: backup larb0/1/2 if needed\n", __func__);
        mm1_backing_up = 1;
        ...
        if (unlikely(get_chip_eco_ver() == CHIP_E1)) {
            xlog_printk(ANDROID_LOG_INFO, TAG, "[%s]: mask M4U irq before power off MMSYS1\n", __func__);
            disable_irq(MT6575_SMI_LARB0_MMU_IRQ_ID);    
            disable_irq(MT6575_SMI_LARB1_MMU_IRQ_ID);    
            disable_irq(MT6575_SMI_LARB2_MMU_IRQ_ID);    
            mt6575_irq_set_polarity(MT6575_SMI_LARB0_MMU_IRQ_ID, MT65xx_POLARITY_LOW);
            mt6575_irq_set_polarity(MT6575_SMI_LARB1_MMU_IRQ_ID, MT65xx_POLARITY_LOW);
            mt6575_irq_set_polarity(MT6575_SMI_LARB2_MMU_IRQ_ID, MT65xx_POLARITY_LOW);
        }
#if 0
    } else if (id == MT65XX_SUBSYS_MM2) {
        xlog_printk(ANDROID_LOG_INFO, TAG, "[%s]: backup larb3 if needed\n", __func__);
        mm2_backing_up = 1;
...
static void mt6575_cg_init(void)
{
    volatile unsigned int reg_val = 0;

    if (clkmgr_debug_mask & DEBUG_INIT) {
        xlog_printk(ANDROID_LOG_INFO, TAG, "[%s]: <== reset clock gating bit\n", __func__);
        dump_clk_regs();
    }
...
static void mt6575_clock_init(void)
{
    int i = 0, j = 0;
    ...
    for (i = 0; i < MT65XX_CLOCK_CATEGORY_COUNT; i++) {
        ...
        if (clkmgr_debug_mask & DEBUG_INIT) {
            xlog_printk(ANDROID_LOG_INFO, TAG, "[%s]: clock_state[%d] = 0x%08x", 
        ...
        for (j = 0; j < mt65xx_clock_number_per_category[i]; j++) {
            ...
            if (clkmgr_debug_mask & DEBUG_INIT) {
                xlog_printk(ANDROID_LOG_INFO, TAG, "[%s]: clock[%d][%d].name = %s, init_state = %s\n", 
...
static void mt6575_pll_init(void)
{
    int i = 0;
    ...
    for (i = 0; i < MT65XX_PLL_COUNT; i++) {
        ...
        if (clkmgr_debug_mask & DEBUG_INIT) {
            xlog_printk(ANDROID_LOG_INFO, TAG, "[%s]: pll[%d].name = %s, init_state = %s\n", 
...
static void mt6575_subsys_init(void)
{
    int i = 0;
    ...
    for (i = 0; i < MT65XX_SUBSYS_COUNT; i++) {
        ...
        if (clkmgr_debug_mask & DEBUG_INIT) {
            xlog_printk(ANDROID_LOG_INFO, TAG, "[%s]: subsys[%d].name = %s, init_state = %s\n", 
...
static void mt6575_mipi_init(void)
{
    int i = 0;
...
void mt6575_clk_mgr_init(void)
{
    if (initialized) {
        xlog_printk(ANDROID_LOG_INFO, TAG, "[%s]: clock manager initialized already!\n", __func__);
        return;
    }
    ...
    initialized = 1;	
    xlog_printk(ANDROID_LOG_INFO, TAG, "[%s]: clock manager initialization success!\n", __func__);
