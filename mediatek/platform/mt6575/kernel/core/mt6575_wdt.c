 ...
 *   IRQ ID 
 *--------------------------------------------------------------------*/
#define AP_RGU_WDT_IRQ_ID    MT6575_WDT_IRQ_ID
...
static struct fiq_glue_handler h;
static void wdt_fiq(struct fiq_glue_handler *h, void *regs, void *svc_sp)
{
...
static int mtk_wdt_probe(struct platform_device *dev)
{
	...
	mt6575_irq_set_sens(AP_RGU_WDT_IRQ_ID, MT65xx_EDGE_SENSITIVE);
        mt6575_irq_set_polarity(AP_RGU_WDT_IRQ_ID, MT65xx_POLARITY_LOW);
#ifndef CONFIG_FIQ_GLUE			
	ret = request_irq(AP_RGU_WDT_IRQ_ID, (irq_handler_t)mtk_wdt_isr, 0, "mtk_watchdog", NULL);
#else
	h.fiq = wdt_fiq;
	ret = request_fiq (AP_RGU_WDT_IRQ_ID, &h);
#endif	
        if(ret != 0)
	{
...
MODULE_AUTHOR("MTK");
MODULE_DESCRIPTION("MT6575 Watchdog Device Driver");
MODULE_LICENSE("GPL");
