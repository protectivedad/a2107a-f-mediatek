...
volatile static int e1_chip = 0;
#define DELAY_COUNT             201
/*
...
void cpu_dormant_abort(void)
{
    ...
    if (e1_chip) {
        ...
        if (cpu_cluster.power_state == STATUS_DORMANT) {
            reg_write(SC_PWR_CON0, 0xE125); /* SRAM wake up */
        } else {
            reg_write(SC_PWR_CON0, 0xC125); /* SRAM power up */
        }
        ...
        if (cpu_cluster.power_state == STATUS_DORMANT) {
            reg_write(SC_PWR_CON2, 0xE525); /* SRAM wake up */
        } else {
            reg_write(SC_PWR_CON2, 0xC525); /* SRAM power up */
        }
        ...
        //reduce dormant abort penalty, limitation: after cpu_power_down be called and abort happened,
        //must call cpu_dormant_abort right away within few cache lines stack size
        stack_addr = cpu.context.banked_registers[18];

        stack_addr -= (CACHE_LINE_SIZE << 2);

        inv_range_pl310(__pa(stack_addr), CACHE_LINE_SIZE << 3, cpu_cluster.l2_address);
        inv_cache_range_v7_l1(stack_addr, stack_addr + (CACHE_LINE_SIZE << 3));

        get_current_stack((void *)&stack_addr);
...
MODULE_AUTHOR("Wen-Shien Chen <ws.chen@mediatek.com>");
MODULE_DESCRIPTION("MT6575 Dormant/Shutdown Mode Driver $Revision: #1 $");
