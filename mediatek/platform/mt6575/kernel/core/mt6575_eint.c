...
static const unsigned int EINT_IRQ = MT6575_EINT_IRQ_ID;
...
static struct device_driver mt6575_eint_drv =
{
    .name = "eint",
...
int mt65xx_eint_init(void)
{
    ...
    for (i = 0; i < DEINT_GROUP_MAX; i++) {
        if (request_irq(MT6575_EINT_DIRECT0_IRQ_ID + i, mt65xx_deint_isr, IRQF_TRIGGER_HIGH, "DEINT", (void *)&deint_group_id[i])) {
            printk(KERN_ERR "DEINT IRQ LINE NOT AVAILABLE!!\n");
        }
    }
#endif

    ret = driver_register(&mt6575_eint_drv);
    if (ret) {
        printk("fail to register mt6575_eint_drv\n");
    }

    ret = driver_create_file(&mt6575_eint_drv, &driver_attr_current_eint);
    ret |= driver_create_file(&mt6575_eint_drv, &driver_attr_current_eint_sens);
    ret |= driver_create_file(&mt6575_eint_drv, &driver_attr_current_eint_pol);
    ret |= driver_create_file(&mt6575_eint_drv, &driver_attr_current_eint_deb);
    ret |= driver_create_file(&mt6575_eint_drv, &driver_attr_current_eint_deb_en);
    if (ret) {
        printk("fail to create mt6575_eint sysfs files\n");
    }
