...
extern void mt6575_uart_update_sysclk(void);
...
int __init dram_overclock_init(void)
{
    ...
    if (ret) {
        printk("fail to create the dram_overclock driver\n");
        return ret;
    }
    ret = driver_create_file(&dram_overclock_drv, &driver_attr_emi_clk_test);
    if (ret) {
        printk("fail to create the dram_overclock sysfs files\n");
        return ret;
    }
