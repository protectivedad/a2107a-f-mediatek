...
S32 pwm_set_easy_config ( struct pwm_easy_config *conf)
{
	...
	switch ( conf->clk_src ) {
		...
		case PWM_CLK_OLD_MODE_32K:
			...
			if ( conf->pwm_no == PWM3 || conf->pwm_no == PWM4 || conf->pwm_no == PWM5 ){
...
int mt6575_pwm_probe ( struct platform_device *pdev)
{
	platform_set_drvdata (pdev, pwm_dev);
	return RSUCCESS;
}

int  mt6575_pwm_remove(struct platform_device *pdev)
{
	if ( ! pdev ) {
	...
	PWMDBG ( "mt6575_pwm_remove\n" );
	return RSUCCESS;
}

void mt6575_pwm_shutdown(struct platform_device *pdev)
{
	printk("mt6575_pwm_shutdown\n");
	return;
}

struct platform_driver pwm_plat_driver={
	.probe = mt6575_pwm_probe,
	.remove = mt6575_pwm_remove,
	.shutdown = mt6575_pwm_shutdown,
	.driver = {
		.name="mt6575-pwm"
	},
};

static int __init mt6575_pwm_init(void)
{
	int ret;
...
static void __exit mt6575_pwm_exit(void)
{
	platform_device_unregister ( &pwm_plat_dev );
	platform_driver_unregister ( &pwm_plat_driver );
}

module_init(mt6575_pwm_init);
module_exit(mt6575_pwm_exit);
