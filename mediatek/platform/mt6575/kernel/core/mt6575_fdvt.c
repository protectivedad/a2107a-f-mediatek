...
#define FDVT_DEVNAME     "mt6575-fdvt"

#define MT6573FDVT_DEBUG
#ifdef MT6573FDVT_DEBUG
#define FDVT_FDVT_DEBUG(fmt, arg...) printk( "[MT6575FDVT] "  fmt, ##arg)
#else
#define FDVT_FDVT_DEBUG(x,...)
#endif

#define MT6573FDVT_PARM
#ifdef MT6573FDVT_PARM
#define FDVTPARMDB(fmt,arg...) printk("[MT6575FDVT]" "%s() - "  fmt, __FUNCTION__  ,##arg)
#else
#define FDVTPARMDB(fmt,arg...)
#endif
...
#define FDVT_TC_RLT_BASE_ADR       (FDVT_BASE+0xA0)
...
{        
    iowrite32(0x00001111,FDVT_ENABLE);   
    ...
    iowrite32(0x2F000000,FDVT_RMAP_0           );   
    iowrite32(0x05230401,FDVT_RMAP_1           );   
    iowrite32(0x000F010B,FDVT_FD_SET           );   
    iowrite32(0x00000000,FDVT_FD_CON_BASE_ADR  );   
    iowrite32(0x00002200,FDVT_TC               );   
    ...
    iowrite32(0x00000000,FDVT_TC_RLT_BASE_ADR  );   
    ...
    iowrite32(0x00000000,FDVT_RESULT           );   
    iowrite32(0x00000000,FDVT_INT_EN           );   
    iowrite32(0x014000F0,FDVT_SRC_WD_HT        );   
    iowrite32(0x00000000,FDVT_SRC_IMG_BASE_ADDR);           
}
...
static int mt_fdvt_clk_ctrl(int en)
{
    //
    if (en) {
        enable_clock(MT65XX_PDN_MM_FD, "FDVT");
        //ISP_CLR_BIT(ISP_G1MEMPDN, 9);
    }
    else {
        disable_clock(MT65XX_PDN_MM_FD, "FDVT");
...
static int MT6573FDVT_SetRegHW(MT6573FDVTRegIO * a_pstCfg)
{
    ...
    u32 i=0;
    if (NULL == a_pstCfg) {
        FDVT_FDVT_DEBUG("Null input argrment \n"); 
        return -EINVAL; 
    }
    ...
    if(copy_from_user((void*)pMT6573FDVTWRBuff.u4Addr, (void *) pREGIO->pAddr, pREGIO->u4Count * sizeof(u32))) {
        FDVT_FDVT_DEBUG("ioctl copy from user failed\n");
        return -EFAULT;
    }
...
{   
    ...
    if (_IOC_NONE != _IOC_DIR(cmd)) {                
        ...
        //else
            //FDVT_FDVT_DEBUG(" ioctlnot write command\n"); 
    }
    //else
        //FDVT_FDVT_DEBUG(" ioctl command = NONE\n"); 

    switch(cmd)
    {  
        case MT6573FDVT_INIT_SETPARA_CMD:
            FDVT_FDVT_DEBUG("[FDVT] FDVT_INIT_CMD\n");
            FDVT_basic_config();            
            break;
        case MT6573FDVTIOC_STARTFD_CMD:   
            //FDVT_FDVT_DEBUG("[FDVT] MT6573FDVTIOC_STARTFD_CMD \n");   
        ...
        case MT6573FDVTIOC_G_WAITIRQ:
            //FDVT_FDVT_DEBUG("[FDVT] MT6573FDVT_WaitIRQ \n");
            ret = MT6573FDVT_WaitIRQ((UINT32 *)pBuff);
            break;
        case MT6573FDVTIOC_T_SET_FDCONF_CMD:  
            //FDVT_FDVT_DEBUG("[FDVT] MT6573FDVT set FD config \n");            
        ...
        case MT6573FDVTIOC_T_SET_SDCONF_CMD:  
            //FDVT_FDVT_DEBUG("[FDVT] MT6573FDVT set SD config \n");            
            SmileDetecteConfig();
            MT6573FDVT_SetRegHW((MT6573FDVTRegIO*)pBuff);
            break;
        case MT6573FDVTIOC_G_READ_FDREG_CMD:    
            //FDVT_FDVT_DEBUG("[FDVT] MT6573FDVT read FD config \n");  
            MT6573FDVT_ReadRegHW((MT6573FDVTRegIO*)pBuff);
            break;                 
        case MT6573FDVTIOC_T_DUMPREG :
            FDVT_FDVT_DEBUG("[FDVT] FDVT_DUMPREG \n");
            MT6573FDVT_DUMPREG();
            break;
        default:
            FDVT_FDVT_DEBUG("[FDVT][ERROR] FDVT_ioctl default case\n");
            break;
    }
    //NOT_REFERENCED(ret);
    //FDVT_FDVT_DEBUG("[FDVT] MT6573FDVT IOCtrol out\n"); 
    if (_IOC_READ & _IOC_DIR(cmd)) {
        if (copy_to_user((void __user *) arg, pBuff , _IOC_SIZE(cmd)) != 0) {
            FDVT_FDVT_DEBUG("copy_to_user failed \n");
            return -EFAULT;
        }
    }
...
static int FDVT_open(struct inode *inode, struct file *file)
{
    ...
    FDVT_FDVT_DEBUG("[FDVT_DEBUG] FDVT_open\n");
    mt_fdvt_clk_ctrl(1);
    if (pBuff != NULL) {
        FDVT_FDVT_DEBUG("pBuff is not null \n");
    }
    if (pread_buf != NULL) {
        FDVT_FDVT_DEBUG("pread_buf is not null \n");
    }
    
    pBuff = kmalloc(buf_size,GFP_KERNEL);

    if(NULL == pBuff) {
        FDVT_FDVT_DEBUG(" ioctl allocate mem failed\n");
        ret = -ENOMEM;
    }
    else
        FDVT_FDVT_DEBUG(" ioctl allocate mem ok\n");
...
{
    FDVT_FDVT_DEBUG("[FDVT_DEBUG] FDVT_flush\n");
...
{
    FDVT_FDVT_DEBUG("[FDVT_DEBUG] FDVT_release\n");
    if (pBuff) {
        kfree(pBuff);
        pBuff = NULL;
    }
...
static int FDVT_probe(struct platform_device *dev)
{
    ...
    struct class_device *class_dev = NULL;
     ...
     FDVT_FDVT_DEBUG("[FDVT_DEBUG] FDVT_probe\n");

    ret = alloc_chrdev_region(&FDVT_devno, 0, 1, FDVT_DEVNAME);
        
    if(ret)
    {
        FDVT_FDVT_DEBUG("[FDVT_DEBUG][ERROR] Can't Get Major number for FDVT Device\n");
    }
    ...
    if ( (ret = cdev_add(FDVT_cdev, FDVT_devno, 1)) < 0) {
        FDVT_FDVT_DEBUG("[FDVT_DEBUG] Attatch file operation failed \n");
        return -EFAULT;
    }
...
//    if (request_irq(MT6575_SMI_LARB1_VAMAU_IRQ_ID, (irq_handler_t)MT6573FDVT_irq, 0, FDVT_DEVNAME, NULL) < 0)
    if (request_irq(MT6575_SMI_LARB1_VAMAU_IRQ_ID, (irq_handler_t)MT6573FDVT_irq, IRQF_TRIGGER_LOW, FDVT_DEVNAME, NULL) < 0)
    {
       FDVT_FDVT_DEBUG("[FDVT_DEBUG][ERROR] error to request FDVT irq\n"); 
    }
    else
    {
       FDVT_FDVT_DEBUG("[FDVT_DEBUG] success to request FDVT irq\n");
    }
    // For Linux 3.0 migration, replace mt65xx_irq_unmask() to enable_irq()   
    // and mark it, due to request_irq() will call enable_irq.
    // mt65xx_irq_unmask(MT6575_SMI_LARB1_VAMAU_IRQ_ID);
    // enable_irq(MT6575_SMI_LARB1_VAMAU_IRQ_ID);
    ...
    FDVT_FDVT_DEBUG("[FDVT_DEBUG] FDVT_probe Done\n");
...
{    
    int i4IRQ = 0;
    FDVT_FDVT_DEBUG("[FDVT_DEBUG] FDVT_driver_exit\n");
    mt_fdvt_clk_ctrl(0);
...
{    
    //BOOL flag;    
    FDVT_FDVT_DEBUG("[FDVT_DEBUG] FDVT_suspend\n");
    //mt_fdvt_clk_ctrl(0);
    /*
...
{   
    //BOOL flag;
    FDVT_FDVT_DEBUG("[FDVT_DEBUG] FDVT_resume\n");
    //mt_fdvt_clk_ctrl(1);
    /*
    if (bMP4HWClockUsed == TRUE)
    {
...
static int __init FDVT_driver_init(void)
{
    ...
    FDVT_FDVT_DEBUG("[FDVT_DEBUG] FDVT_driver_init\n");

    if (platform_device_register(&FDVT_device)){
        FDVT_FDVT_DEBUG("[FDVT_DEBUG][ERROR] failed to register FDVT Device\n");
        ret = -ENODEV;
        return ret;
    }
    
    if (platform_driver_register(&FDVT_driver)){
        FDVT_FDVT_DEBUG("[FDVT_DEBUG][ERROR] failed to register FDVT Driver\n");
        platform_device_unregister(&FDVT_device);
        ret = -ENODEV;
        return ret;
    }
    ...
    FDVT_FDVT_DEBUG("[FDVT_DEBUG] FDVT_driver_init Done\n");
...
MODULE_AUTHOR("Sava, Chan <sava.chan@mediatek.com>");
MODULE_DESCRIPTION("MT6573 FDVT Driver");
MODULE_LICENSE("GPL");
