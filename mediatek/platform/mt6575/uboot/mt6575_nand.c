...
//#include <asm/arch/mt6575_pdn_sw.h>
#include <asm/errno.h>
#include "cust_nand.h"
#include <asm/io.h>
...
/*
static struct nand_ecclayout mt6575_nand_oob = {
...
//-------------------------------------------------------------------------------
static bool mt6575_nand_check_bch_error(u8* pDataBuf, u32 u4SecIndex, u32 u4PageAddr)
{
	bool bRet = true;
...
static bool mt6575_nand_RFIFOValidSize(u16 u2Size)
{
	...
	if(u2Size==0)
	{
		while (FIFO_RD_REMAIN(DRV_Reg16(NFI_FIFOSTA_REG16)))
		{
			timeout--;
			if (0 == timeout){
				printf("mt6575_nand_RFIFOValidSize failed: 0x%x\n",  u2Size);
				return false;
			}
		}
	}
...
//-------------------------------------------------------------------------------
static bool mt6575_nand_ready_for_read(
	struct nand_chip *nand, u32 u4RowAddr, u32 u4ColAddr, bool bFull, u8 *buf)
{
	...
	mt6575_nand_set_autoformat(bFull);
    if (bFull)
	    ECC_Decode_Start();
	...
	struct nand_chip *nand, u32 u4RowAddr, u8 *buf)
{
	...
	//1 FIXED ME: For Any Kind of AddrCycle
	if (!mt6575_nand_set_address(0, u4RowAddr, colnob, rownob)){
		goto cleanup;
	}
...
//-----------------------------------------------------------------------------
static bool mt6575_nand_dma_transfer_data(u8* pDataBuf, u32 u4Size)
{
	u32 timeout = 0xFFFF;
...
static bool mt6575_nand_mcu_transfer_data(u8* pDataBuf, u32 length)
{
	u32 timeout = 0xFFFF;
	u32 i;
	u32* pBuf32;
...
static bool mt6575_nand_read_page_data(u8 *buf, u32 length)
{
#if USE_AHB_MODE
    return mt6575_nand_dma_transfer_data(buf, length);
#else
    return mt6575_nand_mcu_transfer_data(buf, length);
#endif
}

//-----------------------------------------------------------------------------
static bool mt6575_nand_write_page_data(u8* pDataBuf, u32 u4Size)
{
	u32 timeout = 0xFFFF;
#if USE_AHB_MODE	
	NFI_CLN_REG16(NFI_CNFG_REG16, CNFG_BYTE_RW);
...
//-----------------------------------------------------------------------------
static void mt6575_nand_write_fdm_data(u8* pDataBuf, u32 u4SecNum)
{
	u32 i;
	u32* pBuf32 = (u32*)pDataBuf;
	for (i = 0; i < u4SecNum; ++i)
...
int mt6575_nand_exec_read_page_hw(
	struct nand_chip *nand, u32 u4RowAddr, u32 u4PageSize, u8* pPageBuf, u8* pFDMBuf)
{
	bool bRet = true;
	u32 u4SecNum = u4PageSize >> 9;
	
	if (mt6575_nand_ready_for_read(nand, u4RowAddr, 0, true, pPageBuf)) {
		if (!mt6575_nand_read_page_data(pPageBuf, u4PageSize)) {
			bRet = false;
		}
		if (!mt6575_nand_status_ready(STA_NAND_BUSY)) {
			bRet = false;
		...
		if (!mt6575_nand_check_bch_error(pPageBuf, u4SecNum - 1, u4RowAddr)) {
			g_i4ErrNum++;
		}
...
static bool mt6575_nand_exec_read_page(
	struct nand_chip *nand, u32 u4RowAddr, u32 u4PageSize, u8* pPageBuf, u8* pFDMBuf)
{
    ...
    if (!mt6575_nand_exec_read_page_hw(nand, (mapped_block * page_per_block + page_in_block), u4PageSize, pPageBuf, oob))
        return false;
...
//-------------------------------------------------------------------------------
static bool mt6575_nand_exec_write_page(
	struct nand_chip *nand, u32 u4RowAddr, u32 u4PageSize, u8* pPageBuf, u8* pFDMBuf)
{
	bool bRet = true;
	u32 u4SecNum = u4PageSize >> 9;
	if (mt6575_nand_ready_for_write(nand, u4RowAddr, pPageBuf)) {
		mt6575_nand_write_fdm_data(pFDMBuf, u4SecNum);
		if (!mt6575_nand_write_page_data(pPageBuf, u4PageSize)) {
			bRet = false;
		}
		if (!mt6575_nand_check_RW_count(u4PageSize)) {
			bRet = false;
		}
		mt6575_nand_stop_write();
		mt6575_nand_set_command(NAND_CMD_PAGEPROG);
...
static bool mt6575_nand_read_oob_raw(struct nand_chip *chip, u32 page_addr, u32 length, u8 *buf)
{
    u32 sector = 0;
    u32 col_addr = 0;
    ...
    while (length > 0)
    {
        ...
        if (!mt6575_nand_ready_for_read(chip, page_addr, col_addr, false, NULL))
            return false;
        if (!mt6575_nand_mcu_transfer_data(buf, length))
            return false;
        NFI_CLN_REG16(NFI_CON_REG16, CON_NFI_BRD);
        sector++;
        length -= 8;
...
bool nand_block_bad_hw(struct nand_chip *nand, u32 offset)
{
    ...
    if (!mt6575_nand_read_oob_raw(nand, page_addr, OOB_AVAIL_PER_SECTOR, oob_buf))
    {
        printf("mt6575_nand_read_oob_raw return fail\n");
    }
	...
	for (i = 0; i < 4; i++) 
    {
        mt6575_nand_exec_read_page(nand, (page_addr+iaCheckPageID[i]), u4PageSize, nand->buffers->databuf, g_kCMD.au1OOB);
...
#if 0
static int mt6575_nand_part_write(part_dev_t *dev, uchar *src, ulong dst, int size)
{
...
*/
    ...
    while ((size > u4WriteLen) && (u4BlkAddr < u4BlkEnd))
	{
        if(!u4ColAddr)
        {
	    	...
	    	mt6575_nand_set_command(NAND_CMD_ERASE1);
    		mt6575_nand_set_address(0, u4RowAddr, 0, 3);
            mt6575_nand_set_command(NAND_CMD_ERASE2);
		    while(DRV_Reg32(NFI_STA_REG32) & STA_NAND_BUSY);
        }
		...
		if(!res)
		{
			...
			for (; u4RowAddr < u4RowEnd; u4RowAddr++)
			{
				...
				{
					mt6575_nand_exec_write_page(nand, u4RowAddr, u4PageSize, 
...
//-------------------------------------------------------------------------------	
static int mt6575_nand_part_read(part_dev_t *dev, ulong source, uchar *dst, int size)
{
	struct nand_chip *nand = (struct nand_chip *)dev->blkdev;
	uint8_t	res;        // *bbt = nand->bbt;
	u32 u4PageSize  = 1 << nand->page_shift;
	...
	while ((size > u4ReadLen) && (u4BlkAddr < u4BlkEnd))
	{
		...
		if(!res)
		{
			...
			for (; u4RowAddr < u4RowEnd; u4RowAddr++)
			{
				...
				if ((u4ColAddr == 0) && (i4Len == u4PageSize)) 
				{
					mt6575_nand_exec_read_page(nand, u4RowAddr, u4PageSize, 
...
//-------------------------------------------------------------------------------
#if 0
static void mt6575_nand_release_device (struct mtd_info *mtd)
{
...
#endif
//-------------------------------------------------------------------------------
static void mt6575_nand_command_bp(struct mtd_info *mtd, unsigned command, int column, int page_addr)
{  
    ...
    case NAND_CMD_PAGEPROG:
       	if (g_kCMD.pDataBuf || (0xFF != g_kCMD.au1OOB[0])) {
			...
			mt6575_nand_exec_write_page(nand, g_kCMD.u4RowAddr, mtd->writesize,
    ...
    case NAND_CMD_ERASE1:
		...
		mt6575_nand_set_mode(CNFG_OP_ERASE);
		mt6575_nand_set_command(NAND_CMD_ERASE1);
		mt6575_nand_set_address(0,page_addr,0,devinfo.addr_cycle-2);
        break;
    ...
    case NAND_CMD_STATUS:
		...
		mt6575_nand_set_command(NAND_CMD_STATUS);
	...
	case NAND_CMD_READID:      
        ...
        //NFI_SET_REG16(NFI_CNFG_REG16, CNFG_READ_EN);
		mt6575_nand_set_mode(CNFG_OP_SRD);
		mt6575_nand_set_command(NAND_CMD_READID);
		mt6575_nand_set_address(0,0,1,0);
		DRV_WriteReg16(NFI_CON_REG16, CON_NFI_SRD);
		while(DRV_Reg32(NFI_STA_REG32) & STA_DATAR_STATE);
		break; 
    default:
		printf("[ERR] mt6575_nand_command_bp : unknow command %d\n", command);
        BUG();        
        break;
    }
}
...
//-----------------------------------------------------------------------------
static u_char mt6575_nand_read_byte(struct mtd_info *mtd)
{
...
//-----------------------------------------------------------------------------
static void mt6575_nand_read_buf(struct mtd_info *mtd, u_char *buf, int len)
{	
	...
	if (u4ColAddr < u4PageSize) {
		if ((u4ColAddr == 0) && (len >= u4PageSize)) {
			mt6575_nand_exec_read_page(nand, pkCMD->u4RowAddr, u4PageSize, 
									   buf, pkCMD->au1OOB);
			if (len > u4PageSize) {
...
//-----------------------------------------------------------------------------
static void mt6575_nand_write_buf(struct mtd_info *mtd, const u_char *buf, int len)
{
	struct mt6575_CMD* pkCMD = &g_kCMD;
	u32 u4ColAddr = pkCMD->u4ColAddr;
	u32 u4PageSize = mtd->writesize;
...
//-----------------------------------------------------------------------------
static int mt6575_nand_verify_buf(struct mtd_info *mtd, const u_char *buf, int len)
{
 ...
 *
 *****************************************************************************/
static int mt6575_read_multi_page_cache(struct mtd_info *mtd, struct nand_chip *chip,
        uint8_t *buf, int page, int len)
{
    // mt6575_nand_ready_for_read(chip, page, 0, true);
    ...
    while (len > 0)
    {
        ...
        if (len > mtd->writesize)               // remained more than one page
            mt6575_nand_set_command(0x31);      // TODO: add cache read command
        else
            mt6575_nand_set_command(0x3f);      // last page remained

        mt6575_nand_status_ready(STA_NAND_BUSY);

#ifdef USE_AHB_MODE
        mt6575_nand_dma_transfer_data(buf, mtd->writesize);
#else
        mt6575_nand_mcu_transfer_data(buf, mtd->writesize);
#endif
        // get ecc error info
...
//-----------------------------------------------------------------------------
#if 0
static void mt6575_nand_enable_hwecc(struct mtd_info *mtd, int mode)
{	
...
#endif
//-----------------------------------------------------------------------------
static int mt6575_nand_calculate_ecc(struct mtd_info *mtd, const u_char *dat, u_char *ecc_code)
{
...
//-----------------------------------------------------------------------------
static int mt6575_nand_correct_data(struct mtd_info *mtd, u_char *dat, u_char *read_ecc, u_char *calc_ecc)
{
    return g_i4ErrNum;
}

static void mt6575_nand_hwctl(struct mtd_info *mtd, int mode)
{
}
bool getflashid(u16 *id)
{
	...
	mt6575_nand_command_bp(mtd, NAND_CMD_READID, 0,0);
	maf_id = mt6575_nand_read_byte (mtd);
	dev_id = mt6575_nand_read_byte (mtd);
...
//-----------------------------------------------------------------------------
static int mt6575_nand_init(struct nand_chip *this)
{
...
{
	int res = mt6575_nand_init(nand);
    int busw = IO_WIDTH_8 ;
	int id, nand_maf_id,nand_dev_id;
    u8 ext_id1, ext_id2, ext_id3;
    u32 ext_id;
	struct mtd_info *mtd = NULL;	
	nand->select_chip 	= mt6575_nand_select_chip;
	nand->cmdfunc 		= mt6575_nand_command_bp;
	nand->read_byte   	= mt6575_nand_read_byte;
	nand->write_buf   	= mt6575_nand_write_buf;
	nand->dev_ready   	= mt6575_nand_dev_ready;
	//nand->enable_hwecc 	= mt6575_nand_enable_hwecc;
	nand->ecc.calculate     = mt6575_nand_calculate_ecc;
	nand->ecc.correct 	= mt6575_nand_correct_data;
	nand->verify_buf 	= mt6575_nand_verify_buf;
	nand->read_buf  	= mt6575_nand_read_buf;
	//nand->block_markbad = mt6575_nand_default_block_markbad;

	nand->ecc.mode 		= NAND_ECC_HW;
    nand->ecc.hwctl     = mt6575_nand_hwctl;
    nand->ecc.layout    = &nand_oob_64;
    nand->ecc.size      = 512;
	nand->options 	    = NAND_NO_AUTOINCR;

    mt6575_nand_command_bp(mtd, NAND_CMD_READID, 0,0);
    nand_maf_id = mt6575_nand_read_byte (mtd);
    nand_dev_id = mt6575_nand_read_byte (mtd);
    //add for multi-device support
    id = nand_dev_id| (nand_maf_id << 8);
    
    ext_id1 = mt6575_nand_read_byte (mtd);
    ext_id2 = mt6575_nand_read_byte (mtd);
    ext_id3 = mt6575_nand_read_byte (mtd);
...
void board_nand_select_device(struct nand_chip *nand, int chip)
{
	...
	mt6575_nand_select_chip(NULL, chip);
	...
	dev.read   = mt6575_nand_part_read;
