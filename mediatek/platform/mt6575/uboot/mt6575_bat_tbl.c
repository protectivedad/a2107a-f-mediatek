 ...
 * Description:
 * ------------
 *   MT6575 Linux Uboot power scheme
 *
 * Author:
 * -------
 *   James Lo 		(MTK02485)
...
extern int mt6575_detect_powerkey(void);
...
void select_charging_curret_ncp1851(void)
{
    ...
    else if (BMT_status.charger_type == NONSTANDARD_CHARGER)
    {
        ...
        ncp1851_set_iinlim(0x3); //IN current limit at 500mA
        ncp1851_set_ac_current();
...
void ChargerHwInit_ncp1851(void)
{
	...
	if (Enable_BATDRV_LOG == 1) {
		printf("[BATTERY:ncp1851] ChargerHwInit\n" );
	}
...
{
	if (Enable_BATDRV_LOG == 1) {
		printf("[BATTERY] pchr_turn_off_charging_ncp1851!\r\n");
	}
...
int BAT_CheckBatteryStatus(void)
{
    ...
    if (low_bat_boot_display == 0)
    {
        ...
        if (mt6575_detect_key(BACKLIGHT_KEY)) {
            bl_switch = KAL_FALSE;
            bl_switch_timer = 0;
            g_bl_on = 1;
...
int BAT_CheckBatteryStatus_ncp1851(void)
{
    ...
    if (Enable_BATDRV_LOG == 1) {
        printf("[BATTERY:ADC:ncp1851] VCHR:%ld BAT_SENSE:%ld I_SENSE:%ld TBAT:%ld TBAT_vol:%ld \n",
...
void BAT_thread(void)
{
    ...
    if ( upmu_is_chr_det() == KAL_FALSE )
    {
        ...
        printf("[BATTERY] mt6575_power_off !!\n");
        #ifndef NO_POWER_OFF
        mt6575_power_off();
        #endif
        while(1);
...
void BAT_thread_ncp1851(void)
{
    ...
    if(BAT_status == PMU_STATUS_FAIL || g_Battery_Fail)
    {
        ...
        if (Enable_BATDRV_LOG == 1)
        {
            printf("[BATTERY:ncp1851] PMU_STATUS_FAIL \n");
        }
    }
    /* Battery Full */
    /* HW charging done, real stop charging */
        ...
        while (1)
        {
            	...
            	|| mtk_wdt_boot_check()==WDT_BY_PASS_PWK_REBOOT || g_boot_arg->boot_reason==BR_TOOL_BY_PASS_PWK )
            {
                ...
                while ( BMT_status.bat_vol < BATTERY_LOWVOL_THRESOLD )
                {
                    ...
                    if((pwrkey_ready ==true) & mt6575_detect_powerkey()==0 )
                    {
            ...
            if (mt6575_detect_key(BACKLIGHT_KEY) || (!mt6575_detect_powerkey() && press_pwrkey_count > 0))
            {
                bl_switch = KAL_FALSE;
                bl_switch_timer = 0;
                g_bl_on = 1;
