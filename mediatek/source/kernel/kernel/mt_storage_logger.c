...
#define ACCESS_PERMISSION 0666	//666 means that all users can read/write but not execute
...
static uint *storage_logger_mem_pool = NULL;
...
static bool enable_StorageLogger = false;
static bool enable_mmcqd_dump = false;
static bool enable_blklayer_dump = false;
static bool enable_vfs_dump = false;
static bool enable_msdc_dump = false;

/*========USB PART========*/
static bool enable_musb_dump = false;
static bool enable_usb_gadget_dump = false;
/*========================*/
...
static struct class *pclass_storage_logger = NULL;
#endif
...
static struct _loggerMsgFormat storageLoggerFmt[]=
{
	...
	{STORAGE_LOGGER_MSG_IOSCHED1,"%d%Lu%Lu%Lu","nr_iowait[%d][1]iowait_jf[%Lu]total_jf[%Lu]iowait_sum[%Lu]"},
	{STORAGE_LOGGER_MSG_IOSCHED2,"%d%Lu%Lu%Lu%Lu","nr_iowait[%d][2]iowait_jf[%Lu]total_jf[%Lu]io_count[%Lu]iowait_sum[%Lu]"},
...
static struct _loggerMsgFormat usb_logger_format[]=
{
	...
	MSG_ARRAY_INDEX(TXSTATE_END, "%s%s%d%d%04x%d%d", "%s TX/IN %s len %d/%d, txcsr %04x, fifo %d/%d"),
	...
	MSG_ARRAY_INDEX(INTERFACE_DESCRIPTOR, "%02x%02x%02x%02x%02x%02x%02x%02x%02x",	"interface_descriptor, "\
	...
	MSG_ARRAY_INDEX(ENDPOINT_DESCRIPTOR, "%02x%02x%02x%02x%04x%02x",	"endpoint_descriptor, "\
	...
	MSG_ARRAY_INDEX(ACM_SETUP, "%d%02x%02x%04x%04x%d", "acm ttyGS%d req%02x.%02x v%04x i%04x len=%d"),
	MSG_ARRAY_INDEX(ACM_SET_ALT, "%d%d%d%p%p", "acm intf=%d,ctrl_id=%d,data_id=%d,notify=%p,port.in=%p"),
...
#endif
static int storage_logger_dump_write_proc(struct file *file, const char *buffer, unsigned long count, void *data)
{
  char tmpBuf[4];
	...
	if(0 != local_logger_bs) {
		if((local_logger_bs != storage_logger_bufsize)) {
			...
			SLog_MSG("storage_logger_mem_pool_addr:0x%p, local_logger_bs:%d," \
				"storage_logger_bufsize:%d", storage_logger_mem_pool, local_logger_bs, \
				storage_logger_bufsize);
		}
...
static int storage_logger_filename_write_proc(struct file *file, const char *buffer, unsigned long count, void *data)
{
  ...
  if(copy_from_user(tmpBuf, buffer, u4CopySize))
  {
    printk(KERN_INFO "Fail to copy buffer from user space, %s, %d\n", __FUNCTION__, __LINE__);
    return 0;
  }
...
static int storage_logger_iosched_write_proc(struct file *file, const char *buffer, unsigned long count, void *data)
{
	char tmpBuf[4];
	int u4CopySize = 0;
	int tmp = 0;
	...
	{	
		...
		if (iswrapped)
		{
			...
			while ((len+bin_len) >= (readIndex-writeIndex)) {
				...
				if (writeIndex >=  ((storage_logger_bufsize>>2) - (storage_logger_bufsize>>6) ) ) {
						readIndex = 0;
...
static int storage_logger_write_proc(struct file *file, const char *buffer, unsigned long count, void *data)
{
	...
	} else if( 1 == sscanf(acBuf, "%d",&tmp)) {
		SLog_MSG("Your data: %d",tmp);
	} else {
		SLog_MSG("No input");
	}
	return count;
...
static int storage_logger_write_flag(struct file *file, const char *buffer, unsigned long count, void *data)
{
	unsigned long CopySize = 0;
	...
	while (binOffset < writeIndex)
	{
		...
		if(info->type == LOGGER_TYPE_USB) {
			...
			seq_printf(m,"[USB] MsgID:%d,PID:[%s|%d]-%s @%d,[%5lu.%06lu]-", \
					...
					line_cnt, (unsigned long) curtime_low, nanosec_rem / 1000);
		} else {
			seq_printf(m,"MsgID:%d,PID:[%s|%d],%s_%d,[%Lu]-", \
					msg_id, procname,tmp_pid, info->logger_func_string[func_id].FuncName, \
					line_cnt, curtime_low);
		}
...
#ifdef CONFIG_BINARY_PRINTF
		bstr_printf((char *)localbuffer, MAX_SINGLE_LINE_SIZE, fmt, storage_logger_mem_pool+binOffset);
#endif
...
static int storage_logger_proc_single_open(struct file *file, int (*show)(struct seq_file *, void *),	void *data)
{
...
static int __init storage_logger_init(void)
{
	...
	storage_logger_bufsize = 10*1024*1024;
