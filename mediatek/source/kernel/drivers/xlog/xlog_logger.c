...
static ssize_t warn_overflow(struct logger_log *log, struct logger_reader *reader, char __user *buf) {
        int i;
		...
		char head[] = {
            ...
            0x00,0x00,0x00,0x00,
            0x5,'A','E','E',0x0};
        sprintf(&(message[25]), "some logs might have been lost (%ld bytes estimated)",log->sign-reader->sign);
		
        head[0] = strlen(&(message[25]))+6;
        for(i=0;i<25;i++) message[i]=head[i];
        if (copy_to_user(buf, message, strlen(&(message[25]))+26)) return -EFAULT;
        reader->sign = log->sign-(log->sign%log->size)+reader->r_off;
        if(reader->sign>log->sign) reader->sign-=log->size;
...
#define __KSYSTEM_BUF_SIZE 64*1024 
#endif
...
int __xlog_ksystem_printk(const struct xlog_record *rec, ...)
{
	...
	tag_len = strlen(rec->tag_str) + 1;
	memcpy(&xlog_print_buf[1], rec->tag_str, tag_len);
	n = vsnprintf(&xlog_print_buf[tag_len + 1], sizeof(xlog_print_buf) - tag_len - 2, rec->fmt_str, args);
