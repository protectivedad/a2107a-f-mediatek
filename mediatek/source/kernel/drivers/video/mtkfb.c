...
static unsigned int lcd_fps = 6000;
    ...
    for( ;; ) {
        ...
        if(!esd_kthread_pause)
        {
           ...
           {            
                ...
                if(DISP_EsdCheck())
                {
                    MTKFB_LOG("OOps! DISP_EsdRecover dose not work\n");
...
static void mtkfb_update_screen_impl(void)
{
	...
	if (down_interruptible(&sem_overlay_buffer)) {
       	MTKFB_LOG("[FB Driver] can't get semaphore in mtkfb_update_screen_impl()\n");
    }
	else
		down_sem = TRUE;
...
static int mtkfb_update_screen(struct fb_info *info)
{
	MTKFB_FUNC();
    if (down_interruptible(&sem_early_suspend)) {
        MTKFB_LOG("[FB Driver] can't get semaphore in mtkfb_update_screen()\n");
        return -ERESTARTSYS;
    }
...
int mtkfb_set_backlight_level(unsigned int level)
{
	MTKFB_FUNC();
	if (down_interruptible(&sem_flipping)) {
        MTKFB_LOG("[FB Driver] can't get semaphore in mtkfb_set_backlight_level()\n");
        return -ERESTARTSYS;
    }
...
int mtkfb_set_backlight_mode(unsigned int mode)
{
	MTKFB_FUNC();
	if (down_interruptible(&sem_flipping)) {
        MTKFB_LOG("[FB Driver] can't get semaphore in mtkfb_set_backlight_mode()\n");
        return -ERESTARTSYS;
    }
...
static int mtkfb_pan_display_impl(struct fb_var_screeninfo *var, struct fb_info *info)
{
    ...
    if (down_interruptible(&sem_flipping)) {
        MTKFB_LOG("[FB Driver] can't get semaphore in mtkfb_pan_display_impl()\n");
        return -ERESTARTSYS;
    }
...
#if 0
    if (down_interruptible(&sem_early_suspend)) {
        MTKFB_LOG("[FB Driver] can't get semaphore in mtkfb_set_overlay_layer()\n");
        return -ERESTARTSYS;
    }
#else
...
static int mtkfb_get_video_layer(struct fb_info *info, struct fb_overlay_layer *layerInfo)
{
    ...
    if(LCD_STATUS_OK != LCD_Capture_VideoLayer_Query()){
	    MTKFB_LOG("[FB Driver] capture video layer not support\n");
		return -1;
	}
	...
	if (down_interruptible(&sem_early_suspend)) {
        MTKFB_LOG("[FB Driver] can't get semaphore in mtkfb_capture_framebuffer()\n");
        return -ERESTARTSYS;
    }
    if (is_early_suspended) {
          up(&sem_early_suspend);
...
static int mtkfb_ioctl(struct file *file, struct fb_info *info, unsigned int cmd, unsigned long arg)
{
	...
	case MTKFB_POWEROFF:
   	{ 
    	...
    	if (down_interruptible(&sem_early_suspend)) 
		{
        	MTKFB_LOG("[FB Driver] can't get semaphore in mtkfb_early_suspend()\n");
        	return -ERESTARTSYS;
    	}
	...
	case MTKFB_REGISTER_OVERLAYBUFFER:
    {
		...
		MTKFB_LOG("[mtkfb_ioctl]MTKFB_REGISTER_OVERLAYBUFFER\n");
#if defined(MTK_M4U_SUPPORT) 
		...
		else
		{
			...
			{
				MTKFB_LOG("[mtkfb_ioctl Error]MTKFB_REGISTER_OVERLAYBUFFER VA should not be 0x00000000\n");
				return (r);
			}
			c = overlay_buffer_head;
			{
				struct fb_overlay_buffer_list *a;
				a = overlay_buffer_head;
				MTKFB_LOG("[mtkfb_ioctl] before add a node,list node Addr:\n");
				while(a)
				{
					MTKFB_LOG("0x%x,  ", (unsigned int)a);
					a = a->next;
			...
			if(!n){
				MTKFB_LOG("[mtkfb_ioctl] vmalloc failed\n");
				return -ENOMEM;
			}

			if (down_interruptible(&sem_flipping)) {
    	    	MTKFB_LOG("[mtkfb_ioctl] can't get semaphore\n");
        		return -ERESTARTSYS;
   	 		}
			if (down_interruptible(&sem_early_suspend)) {
        		MTKFB_LOG("[mtkfb_ioctl] can't get semaphore\n");
			...
			MTKFB_LOG("[mtkfb_ioctl]MTKFB_REGISTER_OVERLAYBUFFER,allocated mva = 0x%x\n", overlay_mva);
			...
			MTKFB_LOG("[current process ID = %d, current process name:%s]\n", current->tgid, current->comm);
			n->next = NULL;
			c->next = n;
			{
				struct fb_overlay_buffer_list *b;
				b = overlay_buffer_head;
				MTKFB_LOG("[mtkfb_ioctl] after add a node,list node Addr:\n");
	...
	case MTKFB_UNREGISTER_OVERLAYBUFFER:
    {
		...
		MTKFB_LOG("[mtkfb_ioctl]MTKFB_UNREGISTER_OVERLAYBUFFER\n");
#if defined(MTK_M4U_SUPPORT) 
		...
		{
			struct fb_overlay_buffer_list *c,*n;
			MTKFB_LOG("[mtkfb_ioctl]MTKFB_UNREGISTER_OVERLAYBUFFER,va0 = 0x%x\n", overlay_bufferVA);
...
//				ASSERT(n); //n is NULL, but not find matched buffer
			...
			MTKFB_LOG("[mtkfb_ioctl]MTKFB_UNREGISTER_OVERLAYBUFFER,va1 = 0x%x, size = %d\n", 
        	...
        	DISP_DeallocMva(n->buffer.src_vir_addr, n->src_mva, n->buffer.size);
			{
				struct fb_overlay_buffer_list *a;
				a = overlay_buffer_head;
				MTKFB_LOG("[mtkfb_ioctl] before delete a node,list node Addr:\n");
    ...
    case MTKFB_LOCK_FRONT_BUFFER:
        if (down_interruptible(&sem_flipping)) {
            MTKFB_LOG("[FB Driver] can't get semaphore when lock front buffer\n");
            return -ERESTARTSYS;
        }
        return 0;
...
static int mtkfb_probe(struct device *dev)
{
	...
	if(p == NULL){
		...
		p += 4;
		lcd_fps = simple_strtol(p, p+4, 10);
	}
...
static void mtkfb_shutdown(struct device *pdev)
{
    ...
    if (down_interruptible(&sem_early_suspend)) {
        MTKFB_LOG("[FB Driver] can't get semaphore in mtkfb_shutdown()\n");
        return;
    }
...
static void mtkfb_early_suspend(struct early_suspend *h)
{
    ...
    MTKFB_LOG("[FB Driver] enter early_suspend\n");
    ...
    MTKFB_LOG("[FB Driver] leave early_suspend\n");
...
static void mtkfb_late_resume(struct early_suspend *h)
{
    ...
    MTKFB_LOG("[FB Driver] leave late_resume\n");
