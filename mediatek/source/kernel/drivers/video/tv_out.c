...
//#include <mach/sync_write.h>
    ...
    #include <mach/mt6575_pmic_feature_api.h> //PMIC Analog switch select
#else
	#error "unknown arch"
#endif
...
TVOUT_STATUS tvout_setup_video_buffer_list(unsigned int va, unsigned int bufSize, unsigned int * addr_hw_access)
{
#if 0
    tvout_video_buffer* pBufLink = NULL;
    struct list_head *pNode;
		...
		UINT32 width, UINT32 height)
{
    ...
    if(!is_pmem_range((unsigned long*)pa, bufSize))
    {
        ...
        if (TVOUT_STATUS_OK != tvout_setup_video_buffer_list(va, bufSize, &addr_hw_access)) {
